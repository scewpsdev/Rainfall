...
public abstract class Mob : Entity, Hittable, StatusEffectReceiver
{
	...
	public Vector2 actionInput = Vector2.Zero;
	...
	public virtual void onDeath(Entity by)
	{
		...
		if (dropCoins)
		{
			int amount = (int)MathF.Ceiling(maxHealth * 1.5f); // MathHelper.RandomInt(1, Math.Max((int)MathF.Round(maxHealth / 2), 1));
			while (amount > 0)
			{
	...
	void updateMovement()
	{
		...
		if (delta.lengthSquared > 0)
		{
			...
			//if (isGrounded)
			{
				if (Vector2.Rotate(input, -rotation).x > 0)
					direction = 1;
				else if (Vector2.Rotate(input, -rotation).x < 0)
					direction = -1;
