...
public class HUD
{
	...
	{
		lastBossDefeated = Time.currentTime;
	}
	...
	void renderPopup()
	{
		...
		{
			text = new string[GameState.instance.currentBoss.Length];
			for (int i = 0; i < GameState.instance.currentBoss.Length; i++)
				text[i] = GameState.instance.currentBoss[i].displayName;
			elapsed = (Time.currentTime - GameState.instance.bossFightStarted) / 1e9f;
		}
		else 
		*/
		if (lastLevelSwitch != -1 && (Time.currentTime - lastLevelSwitch) / 1e9f < POPUP_DURATION && levelName != null)
		{
			text = [levelName];
			elapsed = (Time.currentTime - lastLevelSwitch) / 1e9f;
		}
		if (lastBossDefeated != -1 && (Time.currentTime - lastBossDefeated) / 1e9f < POPUP_DURATION)
		{
			text = ["BOSS DEFEATED"];
			elapsed = (Time.currentTime - lastBossDefeated) / 1e9f;
		}
		...
		if (enabled)
		{
			...
			*/
			...
			if (player.isAlive)
			{
				...
				{
					Vector2 pos = GameState.instance.camera.worldToScreen(player.position + player.collider.center + player.lookDirection * 1.5f);
