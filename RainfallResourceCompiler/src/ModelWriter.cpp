...
static void WriteCustomProperty(FileWriterI* writer, CustomProperty* property, Error* err)
{
	write(writer, property->name, err);
	write(writer, property->type, err);
	if (property->type <= CustomPropertyType::Double)
	{
		write(writer, (uint64_t)property->data, err);
	}
	else if (property->type == CustomPropertyType::String)
	{
		char* str = (char*)property->data;
		int len = (int)strlen(str);
		write(writer, len, err);
		write(writer, str, (len * 4 + 3) / 4, err);

	}
	else if (property->type == CustomPropertyType::Vector3)
	{
		write(writer, property->data, 3 * 4, err);
	}
}
...
static void WriteNode(FileWriterI* writer, const NodeData& node, Error* err)
{
	...
	write(writer, node.numProperties, err);
	if (node.numProperties)
	{
		for (int i = 0; i < node.numProperties; i++)
		{
			WriteCustomProperty(writer, &node.properties[i], err);
		}
	}
}
