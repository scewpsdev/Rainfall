...
static bool SkipToken(Parser* parser, int tokenType)
{
	...
	else
	{
		...
		SnekErrorLoc(parser->context, parser->lexer->input.state, "Expected token '%c'(%d), got '%.*s'", tokenType, tokenType, tok.len, tok.str);
		parser->failed = true;
		return false;
	}
}
...
static AST::Type* ParseFunctionType(Parser* parser, AST::Type* elementType)
{
	...
	if (NextTokenIs(parser, '('))
	{
		...
		while (upcomingParamType)
		{
			...
			else
			{
				if (AST::Type* paramType = ParseType(parser))
				{
					if (NextTokenIs(parser, '.'))
					{
						if (NextTokenIs(parser, '.', 1) && NextTokenIs(parser, '.', 2))
						{
							varArgs = true;
							upcomingParamType = false;
		...
		auto functionType = new AST::FunctionType(parser->module, location, elementType, paramTypes, varArgs, varArgsType);
...
static AST::Expression* ParseAtom(Parser* parser)
{
	...
	else if (NextTokenIs(parser, TOKEN_TYPE_FLOAT_LITERAL) || NextTokenIs(parser, TOKEN_TYPE_DOUBLE_LITERAL))
	{
		...
		return new AST::FloatingPointLiteral(parser->module, inputState, value, processed, token.type == TOKEN_TYPE_DOUBLE_LITERAL);
	}
	else if (NextTokenIs(parser, TOKEN_TYPE_CHAR_LITERAL))
	{
		...
		for (int i = 0; i < tok.len; i++) {
			...
			if (c == '\\' && i < tok.len - 1) {
				...
				case '"': c = '"'; break;
				default:
					SnekWarnLoc(parser->context, parser->lexer->input.state, "Undefined escape character '\\%c'", tok.str[i]);
					break;
				}
			}
			value = c;
			...
			if (i < tok.len - 1) {
				SnekWarnLoc(parser->context, parser->lexer->input.state, "Invalid character literal length: %d", tok.len - i);
			}
			break;
		}
		...
		else
		{
			...
			for (int i = 0; i < token.len; i++)
			{
				...
				if (c == '\\')
				{
					...
					case '0': c = '\0'; break;
					default:
						SnekWarnLoc(parser->context, parser->lexer->input.state, "Undefined escape character '\\%c'", token.str[i]);
						break;
					}
				}
				buffer << c;
	...
	else if (NextTokenIs(parser, '('))
	{
		...
		if (AST::Expression* compoundValue = ParseExpression(parser)) // Compound
		{
			...
			{
				Token tok = NextToken(parser);
				SnekErrorLoc(parser->context, parser->lexer->input.state, "Expected ')': %.*s", tok.len, tok.str);
				parser->failed = true;
				return nullptr;
			}
		}
		...
		{
			Token tok = NextToken(parser);
			SnekErrorLoc(parser->context, parser->lexer->input.state, "Expected an expression: %.*s", tok.len, tok.str);
			parser->failed = true;
			return nullptr;
		}
	}
	...
	else if (NextTokenIsKeyword(parser, KEYWORD_TYPE_FOR))
	{
		...
		//if (NextTokenIs(parser, '('))
		{
			...
			else if (NextTokenIsKeyword(parser, KEYWORD_TYPE_CONSTANT) || NextTokenIsKeyword(parser, KEYWORD_TYPE_VARIABLE))
			{
				...
				{
					SnekErrorLoc(parser->context, inputState, "Expected identifier");
					return nullptr;
				}
			}
			else
			{
				SnekErrorLoc(parser->context, inputState, "Expected iterator or initializer statement");
				return nullptr;
			}
		}
		/*
...
static AST::Declaration* ParseDeclaration(Parser* parser)
{
				...
				while (HasNext(parser) && upcomingMember)
				{
					...
					if (AST::Type* type = ParseType(parser))
					{
						...
						while (upcomingField && HasNext(parser))
						{
							...
							{
								char* name = GetTokenString(NextToken(parser));
								char* semantic = nullptr;

								if (NextTokenIs(parser, ':'))
								{
									NextToken(parser); // :
									semantic = GetTokenString(NextToken(parser));
								}

								AST::StructField* field = new AST::StructField(parser->module, fieldInputState, type, name, fields.size, semantic);
								fields.add(field);
							}
							else
							{
					...
					if (AST::Type* type = ParseType(parser))
					{
						if (NextTokenIs(parser, TOKEN_TYPE_IDENTIFIER))
						{
							...
							if (NextTokenIs(parser, '('))
							{
								...
								while (HasNext(parser) && upcomingDeclarator)
								{
									...
									else if (AST::Type* paramType = ParseType(parser))
									{
										...
										else
										{
											...
											{
												SnekErrorLoc(parser->context, parser->lexer->input.state, "Function parameter declaration needs a name");
												parser->failed = true;
											}
										}
									}
								...
								AST::Method* method = new AST::Method(parser->module, inputState, flags, endInputState, name, type, paramTypes, paramNames, body, false, {});
								methods.add(method);
							}
							else
							{
						...
						else
						{
							if (type->typeKind == AST::TypeKind::Function && ((AST::FunctionType*)type)->returnType->typeKind == AST::TypeKind::NamedType && strcmp(((AST::NamedType*)((AST::FunctionType*)type)->returnType)->name, className) == 0)
							{
								...
								constructor = new AST::Constructor(parser->module, inputState, flags, endInputState, nullptr, constructorReturnType, paramTypes, paramNames, body, false, {});
							}
							else
							{
								Token tok = NextToken(parser);
	...
	else if (NextTokenIsKeyword(parser, KEYWORD_TYPE_MODULE))
	{
		...
		{
			Token tok = NextToken(parser);
			SnekErrorLoc(parser->context, parser->lexer->input.state, "Expected a module name: %.*s", tok.len, tok.str);
			parser->failed = true;
			return NULL;
		}
	}
		...
		if (NextTokenIs(parser, TOKEN_TYPE_IDENTIFIER))
		{
			...
			if (NextTokenIs(parser, '('))
			{
				...
				while (HasNext(parser) && upcomingDeclarator)
				{
					...
					else if (NextTokenIsKeyword(parser, KEYWORD_TYPE_OUT))
					{
						NextToken(parser);
						out = true;
					}
				...
				*/
				...
				function = new AST::Function(parser->module, inputState, flags, endInputState, name, returnType, paramTypes, paramNames, paramFlags, body, isGeneric, genericParams);
