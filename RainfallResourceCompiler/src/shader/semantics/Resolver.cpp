...
static bool ResolveMalloc(Resolver* resolver, AST::Malloc* expr)
{
	...
	if (ResolveType(resolver, expr->dstType))
	{
		...
		else if (expr->dstType->typeID->typeKind == AST::TypeKind::Class)
		{
			...
			{
				if (expr->arguments.size < constructor->paramTypes.size || expr->arguments.size > constructor->paramTypes.size)
				{
...
static bool ResolveFunctionHeader(Resolver* resolver, AST::Function* decl)
{
	...
	else
	{
		...
		*/
		...
		decl->functionType = GetFunctionType(returnType, numParams, paramTypes, false, nullptr, false, nullptr, decl);
...
static bool ResolveClassMethodHeader(Resolver* resolver, AST::Method* method, AST::Class* decl)
{
	...
	method->functionType = GetFunctionType(returnType, numParams, paramTypes, false, nullptr, true, decl->type, method);
...
static bool ResolveClassConstructorHeader(Resolver* resolver, AST::Constructor* constructor, AST::Class* decl)
{
	...
	constructor->functionType = GetFunctionType(returnType, numParams, paramTypes, false, nullptr, true, decl->type, constructor);
