...
static int GetMetadataSize(aiMetadataEntry* metadata)
{
	if (metadata->mType <= AI_DOUBLE)
	{
		int sizes[AI_DOUBLE + 1] = { 1, 4, 8, 4, 8 };
		return sizes[metadata->mType];
	}
	else if (metadata->mType == AI_AISTRING)
	{
		aiString* str = (aiString*)metadata->mData;
		return str->length + 1;
	}
	else if (metadata->mType == AI_AIVECTOR3D)
	{
		return 3 * 4;
	}
	__debugbreak();
...
static void PrintMetadata(aiMetadata* data)
{
	for (int i = 0; i < data->mNumProperties; i++)
	{
		...
		else if (value->mType == AI_AIMETADATA)
		{
			aiMetadata* child = (aiMetadata*)value->mData;
			PrintMetadata(child);
		}
	}
}
...
static void ProcessNode(NodeData& node, SceneData& scene, aiNode* ainode, int& idCounter, std::unordered_map<aiNode*, int>& nodeMap)
{
	...
	if (ainode->mMetaData)
	{
		...
		for (int i = 0; i < ainode->mMetaData->mNumProperties; i++)
		{
			...
			CustomProperty& property = node.properties[node.numProperties];

			strcpy(property.name, key->C_Str());
			property.type = (CustomPropertyType)value->mType;
			if (value->mType <= AI_DOUBLE)
				memcpy(&property.data, value->mData, GetMetadataSize(value));
			else if (value->mType == AI_AISTRING)
			{
				aiString* str = (aiString*)value->mData;
				property.data = new char[str->length + 1];
				memset(property.data, 0, str->length + 1);
				strcpy((char*)property.data, str->C_Str());
			}
			else if (value->mType == AI_AIVECTOR3D)
			{
				property.data = new Vector3();
				memcpy(property.data, value->mData, sizeof(Vector3));
			}
			else
			{
				continue;
...
static aiColor4D LinearToSRGB(aiColor4D color)
{
	float gamma = 2.2f;
	float r = powf(color.r, 1.0f / gamma);
	float g = powf(color.g, 1.0f / gamma);
	float b = powf(color.b, 1.0f / gamma);
	return aiColor4D(r, g, b, color.a);
}
...
static void ProcessLight(LightData& light, aiLight* ailight)
{
	...
	light.color = Vector3{ ailight->mColorDiffuse.r * 0.001f, ailight->mColorDiffuse.g * 0.001f, ailight->mColorDiffuse.b * 0.001f };
}
