...
public class Player : Creature
{
		...
		: base(EntityType.Get("player"))
	{
		...
		cape = new Cloth(capeMesh.getMeshData(0), animator, clothInvMasses, new ClothParams(0), new Vector3(0, 2, 0), Quaternion.Identity);
	}
	...
	void updateMovement()
	{
		...
		if (Input.IsKeyDown(KeyCode.S))
			movementInput.z++;
		movementInput = Quaternion.AxisAngle(Vector3.Up, camera.yaw) * movementInput;
		...
		animator.getRootMotion(animator.currentAnimation, out Vector3 rootMotion, out Quaternion _, out bool hasLooped);
		rootMotion = Quaternion.AxisAngle(Vector3.Up, MathF.PI) * rootMotion;
		...
		yaw = Mathf.LerpAngle(yaw, directionDst, 10 * Time.deltaTime);
	...
	void updateAnimations()
	{
		...
		if (isGrounded)
		{
			...
			{
				float animationSpeed = Mathf.Clamp(currentSpeed / speed, 0, 2) * 0.6f;
	...
	public override void update()
	{
		...
		audio.setPosition(position + Vector3.Up);
	}
