...
public class Mob : Creature
{
	...
	void onDeath(Vector3 knockback, RigidBody hitbox)
	{
		...
		for (int i = 0; i < itemDrops.Count; i++)
		{
			...
			if (r < itemDrops[i].dropChance)
			{
				...
				//Quaternion rotation = Quaternion.AxisAngle(Mathf.RandomVector3(-1, 1).normalized, Mathf.RandomFloat(0, 2 * MathF.PI));
		...
		if (hitboxes != null)
		{
			...
			{
				Vector3 localVelocity = Quaternion.AxisAngle(Vector3.Up, yaw).conjugated * velocity;
	...
	void updateMovement()
	{
		...
		animator.getRootMotion(animator.currentAnimation, out Vector3 rootMotionPosition, out Quaternion rootMotionRotation, out bool hasLooped);
		...
		velocity = Quaternion.AxisAngle(Vector3.Up, yaw) * velocity;
	...
	public override void draw(GraphicsDevice graphics)
	{
		...
		else
		{
			...
			if (renderHealthbar && (Time.currentTime - lastHitTime) / 1e9f < HEALTHBAR_SHOW_DURATION)
			{
				int width = 120;
				int height = 6;
				Vector2i center = Mathf.WorldToScreenSpace(position + Vector3.Up * 2, Renderer.pv, Display.viewportSize);
