...
public class HostileAI : AI
{
	...
	void runToTarget(Vector3 toTarget, float delta)
	{
		...
		stamina = Mathf.Clamp(stamina + 5 * delta, 0, 100);
	...
	void updateTargetFollow(float delta)
	{
		...
		PhysicsHit? hit = Physics.Raycast(mob.position + Vector3.Up, toTarget / distanceToTarget, distanceToTarget);
	...
	void updatePatrol(float delta)
	{
		if (!lookForTarget())
		{
			...
			else
			{
				...
				PhysicsHit? hit = Physics.Raycast(mob.position + Vector3.Up, toPatrolTarget / distanceToPatrolTarget, distanceToPatrolTarget);
