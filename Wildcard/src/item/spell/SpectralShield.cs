...
class SpectralShieldEntity : Entity, Hittable
{
	...
	public override void render()
	{
		...
		Vector4 color = (Time.currentTime - lastBlock) / 1e9f < cooldown ? new Vector4(0.2f, 0.2f, 0.2f, 1.0f) : Vector4.One;
		if (visible)
			Renderer.DrawSprite(position.x - 0.5f, position.y - 0.5f, LAYER_BG, 1, 1, angle, sprite, false, MathF.Cos(angle) < 0, color, true);
	}
	...
	public bool hit(float damage, Entity by = null, Item item = null, string byName = null, bool triggerInvincibility = true, bool buffedHit = false)
	{
		...
		if ((Time.currentTime - lastBlock) / 1e9f > cooldown)
		{
			...
			if (by != null && by is Mob && !(by as Mob).isStunned)
			{
				Mob mob = by as Mob;
				mob.stun();
				mob.addImpulse((mob.center - position).normalized * 20 + Vector2.Up);
				...
				Audio.PlayOrganic(Item.parryHit, new Vector3(position, 0));
