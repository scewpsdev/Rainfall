...
public abstract class Item
{
	...
	public Vector3 trailSpriteColor = Vector3.One;
	...
	static Item CreateRandom(List<Item> items, Random random, float meanValue)
	{
		...
		for (int i = 0; i < items.Count; i++)
		{
			float gaussian = items[i].getValue() <= meanValue ?
				MathHelper.Gaussian(items[i].getValue() / meanValue - 1, 1, 1.0f) :
				MathHelper.Gaussian((items[i].getValue() - meanValue) / 8, 0, 1);
