...
public class SaveFile
{
	...
	public static SaveFile Load(int saveID)
	{
...
#endif
		{
			...
			{
				LoadItemList(stashedItemsArray, out Item[] stashedItems);
				Debug.Assert(save.stashedItems.Count == 0);
				save.stashedItems.AddRange(stashedItems);
			}
	...
	static RunData LoadRun(DatObject run)
	{
		...
		LoadItemList(items0, out data.handItems);
		...
		LoadItemList(items1, out data.activeItems);
		...
		LoadItemList(items2, out data.passiveItems);
	...
	static void LoadItemList(DatArray itemsArray, out Item[] items)
	{
		items = new Item[itemsArray.size];
		...
		if (file.getInteger("num_active_items", out int numActiveItems))
		{
			for (int i = 0; i < numActiveItems; i++)
			{
				...
				if (activeItemIdx != -1)
				{
					...
					player.activeItems[i].onEquip(player);
				}
			}
		}
		...
		if (file.getInteger("num_spell_items", out int numSpellItems))
		{
			for (int i = 0; i < numSpellItems; i++)
			{
				...
				if (spellItemIdx != -1)
				{
					...
					player.spellItems[player.spellItems.Count - 1].onEquip(player);
				}
			}
		}
		...
		if (file.getInteger("num_passive_items", out int numPassiveItems))
		{
			for (int i = 0; i < numPassiveItems; i++)
			{
				...
				if (passiveItemIdx != -1)
				{
					...
					player.passiveItems[player.passiveItems.Count - 1].onEquip(player);
				}
			}
		}
	...
	public static DatFile Save(SaveFile save)
	{
		...
		for (int i = 0; i < save.highscores.Length; i++)
		{
			...
			run.addArray("items0", SaveItemList(save.highscores[i].handItems));
			run.addArray("items1", SaveItemList(save.highscores[i].activeItems));
			run.addArray("items2", SaveItemList(save.highscores[i].passiveItems));
	...
	static DatArray SaveItemList(Item[] items)
	{
		DatValue[] itemValues = new DatValue[items.Length];
		for (int i = 0; i < items.Length; i++)
		{
			DatObject itemObj;
			if (items[i] != null)
			{
				itemObj = new DatObject()
				{
					fields =
					[
						new DatField("type", new DatValue(items[i].name, DatValueType.Identifier)),
						new DatField("stack_size", new DatValue(items[i].stackSize))
					]
				};
			...
			{
				itemObj = new DatObject() { fields = [] };
			}
			itemValues[i] = new DatValue(itemObj);
		}
		return new DatArray(itemValues);
	}
	...
	public static void SaveQuest(DatObject obj, Quest quest, string npcName)
	{
		if (!obj.getArray("quests", out DatArray quests))
