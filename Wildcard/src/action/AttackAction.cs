...
public class AttackAction : EntityAction
{
	...
	public override void onStarted(Player player)
	{
		...
		if (anim != AttackAnim.Stab)
			trail = new WeaponTrail(20, weapon.trailSprite, new Vector4(weapon.trailSpriteColor, 0.5f), true, getWeaponTip(player, 0), getWeaponTip(player, 0, 0.99f));
	}
		...
		if (hit.entity != null && hit.entity != player && hit.entity is Hittable && !hitEntities.Contains(hit.entity))
		{
			...
			if (hittable.hit(damage, player, weapon, null, true, critical))
			{
				...
				//if (!player.isGrounded)
				{
					...
					player.velocity.y = MathF.Max(player.velocity.y, downwardsFactor * /*player.jumpPower * 0.75f*/ 10);
				}
	...
	void checkHit(Vector2 origin, float progress, Player player)
	{
		...
		if (trail != null)
		{
			...
			if (anim == AttackAnim.SwingSideways)
				thickness = MathHelper.Remap(Vector2.Dot(direction, Vector2.Rotate(Vector2.Right, currentAngle) * new Vector2(MathF.Sign(direction.x), 1)), -1, 1, 0.8f, 0.2f);
			else if (anim == AttackAnim.SwingOverhead)
				thickness = MathHelper.Remap(Vector2.Dot(direction, Vector2.Rotate(Vector2.Right, currentAngle) * new Vector2(MathF.Sign(direction.x), 1)), -1, 1, 0.99f, 0.4f);
	...
	public override void update(Player player)
	{
		...
		float actionMovementDst = MathHelper.Smoothstep(0, 1, currentProgress) / weapon.attackRate;
