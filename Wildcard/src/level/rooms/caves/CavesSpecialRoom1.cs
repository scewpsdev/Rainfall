...
public class CavesSpecialRoom1 : RoomEntity
{
	public CavesSpecialRoom1(Room room)
		: base(room)
	{
	...
	public override void place(RoomBiomeGenerator generator)
	{
		...
		{
			generator.placeEntity(new IronDoor(), room.position + room.doorways[1].position);
		}
		...
		{
			generator.placeEntity(new IronDoor(), room.position + room.doorways[2].position);
		}
		...
		if (generator.random.NextSingle() < 0.8f)
			generator.spawnNPC(room.position + new Vector2i(MathHelper.RandomInt(2, 5, generator.random), 1));
		else
		{
			/*
