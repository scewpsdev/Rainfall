...
public class Room
{
	...
	public Doorway getDoorway(Vector2i position)
	{
		for (int i = 0; i < doorways.Count; i++)
		{
			if (doorways[i].room.position + doorways[i].position == position)
				return doorways[i];
		}
		return null;
	}
	...
	{
		return position.x >= x + 1 && position.x <= x + width - 1 &&
			position.y >= y + 0.5f && position.y <= y + height - 0.5f;
	}

	public bool containsEntity(Entity entity)
	{
		return containsPosition(entity.position);
	}
}
...
public partial class LevelGenerator
{
	...
	public float getLootValue(Vector2 position)
	{
		...
		{
			return MathHelper.Lerp(level.minLootValue, level.maxLootValue, random.NextSingle());
		}
	}
