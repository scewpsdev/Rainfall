...
public partial class LevelGenerator
{
	...
	public void generateDungeons(string seed, out Level[] areaDungeons)
	{
		...
		areaDungeons[0] = new Level(9, "dungeons1", "Weeping Catacombs", 60, 40, null, 20, 50) { ambientLight = ambience };
		areaDungeons[1] = new Level(10, "dungeons2", "", 40, 50, null, 22, 60) { ambientLight = ambience };
		areaDungeons[2] = new Level(11, "dungeons3", "", 50, 50, null, 25, 70) { ambientLight = ambience };
		areaDungeons[3] = new Level(12, "dungeons4", "", 40, 60, TileType.bricks, 27, 80) { ambientLight = ambience };
		areaDungeons[4] = new Level(-1, "dungeons_boss_room", "Forgotten Chamber", 40, 20, TileType.bricks, 27, 80) { ambientLight = ambience }; // loot value will affect what the blacksmith sells in the hub
	...
	public List<NPC> getDungeonNPCList()
	{
		...
		if (!QuestManager.tryGetQuest("logan", "logan_quest", out Quest loganQuest) || loganQuest.state != QuestState.InProgress)
			npcs.Add(new Logan(random, level, 25) /*NPCManager.logan*/);
	...
	void generateDungeonFloor(string seed, int floor, bool spawnStartingRoom, bool spawnBossRoom, Level level, Level nextLevel, Level lastLevel, Door entrance, Func<List<Mob>> createEnemy)
	{
		...
		*/
		...
		if (QuestManager.tryGetQuest("logan", "logan_quest", out Quest loganQuest) && loganQuest.state == QuestState.InProgress)
		{
			if (level == GameState.instance.world.areaDungeons[GameState.instance.world.areaDungeons.Length - 2])
			{
				spawnRoomObject(rooms, rooms.Count, false, (Vector2i pos, Random random, Room room) =>
				{
					spawnItem(pos.x, pos.y, [new LostScroll()]);
				});
			}
		}
