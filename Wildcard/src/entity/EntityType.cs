...
public static class EntityType
{
	...
	static Dictionary<uint, int> colorMap = new Dictionary<uint, int>();
	...
	public static void InitTypes()
	{
		...
		InitType<BrokenWanderer>("broken_wanderer");
		...
		InitType<Trampoline>("trampoline", 0xFFFF7F00);
	...
	static void InitType<T>(string name, uint mapColor = 0) where T : Entity, new()
	{
		...
		if (mapColor != 0)
			colorMap.Add(mapColor, createFuncs.Count - 1);
	}
	...
	public static Entity CreateInstance(uint color)
	{
		if (colorMap.TryGetValue(color, out int idx))
			return createFuncs[idx].Invoke();
		return null;
	}
}
