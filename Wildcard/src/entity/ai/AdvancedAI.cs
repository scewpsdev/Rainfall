...
public class AdvancedAI : AI
{
	...
	public AIAction addAction(string name, string animation, float charge, float duration, float cooldown, float speed, float maxDistance, float minDistance = 0, float rarity = 1)
	{
		AIAction action = addAction(animation, charge, duration, cooldown, speed, maxDistance);
		action.name = name;
		return action;
	}
	...
	public AIAction addAlertAction()
	{
		AIAction freeze = addAction("freeze", "idle", 0, alertFreeze, 0, 0, 0);
		freeze.onStarted = (AIAction action) =>
		{
			mob.level.addEntity(new TextAlert("!") { lifetime = freeze.duration }, mob.position + new Vector2(mob.collider.center.x, mob.collider.max.y));
		};
		return freeze;
	}

	public AIAction addSearchingAction()
	{
		AIAction freeze = addAction("searching", "idle", 0, alertFreeze, 0, 0, 0);
		freeze.onStarted = (AIAction action) =>
		{
			mob.level.addEntity(new TextAlert("?") { lifetime = freeze.duration }, mob.position + new Vector2(mob.collider.center.x, mob.collider.max.y));
		};
	...
	protected override void onTargetSwitched(Entity newTarget)
	{
		if (target == null && newTarget != null)
			triggerAction(getAction("freeze"));
		else if (target != null && newTarget == null)
			triggerAction(getAction("searching"));
	}
	...
	void updateTargetFollow()
	{
		...
		if (state == AIState.Default)
		{
			if (useAStar)
			{
				if (Utils.RunEverySeconds(0.2f, "updateTargetFollow"))
				{
					Vector2i currentTile = (Vector2i)Vector2.Floor(mob.center);
				...
				if (currentPath.Count > 0 && currentAction == null)
				{
					...
					float xdelta = nextTile.x + 0.5f - mob.center.x;
					float ydelta = nextTile.y + 0.5f - mob.center.y;
					...
					{
						currentPath.RemoveAt(currentPath.Count > 1 ? 1 : 0);
						Console.Write("a");
					}
				}
			}
