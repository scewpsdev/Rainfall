...
public class Blacksmith : NPC, WorldEventListener
{
	...
	public override void init(Level level)
	{
		...
		GameState.instance.world.worldEventListeners.Add(this);
	}
	...
	public override void onLevelSwitch(Level newLevel)
	{
		...
		if (newLevel != level && level != GameState.instance.world.hub)
			remove();
	}
	...
	{
		clearShop();
		populateShop(WorldManager.instance.generator.random, 8, 10, boss.level.avgLootValue * 2, ItemType.Weapon, ItemType.Shield, ItemType.Armor, ItemType.Ammo);
		buysItems = true;

		if (GameState.instance.world.areaCaves.Contains(boss.level))
		{
			setInititalDialogue("""
		...
		else if (GameState.instance.world.areaMines.Contains(boss.level))
		{
			setInititalDialogue("""
				You're still alive?
				Hah.
		...
		else if (GameState.instance.world.areaDungeons.Contains(boss.level))
		{
			setInititalDialogue("""
