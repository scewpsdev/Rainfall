...
public class Logan : NPC
{
	...
	public Logan(Random random, Level level, float lootValue)
			: base("logan")
	{
		...
		if (progression.initialDialogue == null && progression.loganQuest.state == QuestState.InProgress && level == GameState.instance.world.hub)
		{
			setOneTimeInititalDialogue("""
		...
		{
			populateShop(random, 5, 10, lootValue, ItemType.Potion, ItemType.Staff, ItemType.Spell, ItemType.Scroll);
			buysItems = true;
			//canAttune = true;
			canIdentify = true;
		}
	...
	public Logan()
		: this(Random.Shared, GameState.instance.level, 15)
	{
	}
