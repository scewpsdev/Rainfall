...
public class PlayerCamera : Entity
{
	...
	{
		float x = Mathf.Remap(pos.x, position.x - 0.5f * width, position.x + 0.5f * width, 0, 1) * Renderer.UIWidth;
		float y = Mathf.Remap(pos.y, position.y - 0.5f * height, position.y + 0.5f * height, 1, 0) * Renderer.UIHeight;
		return new Vector2(x, y);
	}

	public Vector2 screenToWorld(Vector2i pos)
	{
		float x = Mathf.Remap((pos.x + 0.5f) / Renderer.UIWidth, 0, 1, position.x - 0.5f * width, position.x + 0.5f * width);
		float y = Mathf.Remap((pos.y + 0.5f) / Renderer.UIHeight, 1, 0, position.y - 0.5f * height, position.y + 0.5f * height);
		return new Vector2(x, y);
	}
	...
	{
		width = Wildcard.instance.width / 8.0f; // Display.width / (float)scale / 16.0f;
		height = Wildcard.instance.height / 8.0f; // Display.height / (float)scale / 16.0f;
		...
		*/
		...
		if (width < level.width)
			position.x = Mathf.Clamp(position.x, x0, x1);
		else
			position.x = 0.5f * level.width;

		if (height < level.height)
			position.y = Mathf.Clamp(position.y, y0, y1);
		else
