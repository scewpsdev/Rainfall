...
public class RoomDefSet
{
	...
	public RoomDefSet(string path)
	{
		...
		for (int i = 0; i < numRoomDefs; i++)
		{
			...
			{
				DoorDef doorDef = def.doorDefs[j];
				doorDef.position.y = def.height - doorDef.position.y - 1;
				doorDef.direction.y *= -1;
				def.doorDefs[j] = doorDef;
			}
			roomDefs.Add(def);
...
public class Room
{
	...
	{
		int xoffset = random.Next() % this.width;
		int yoffset = random.Next() % this.height;
		for (int i = 0; i < this.width; i++)
		{
			int x = this.x + (xoffset + i) % this.width;
			for (int j = 0; j < this.height; j++)
			{
				int y = this.y + (yoffset + j) % this.height;
	...
	{
		return entity.position.x + entity.collider.min.x >= x + 1 && entity.position.x + entity.collider.max.x <= x + width - 1 &&
			entity.position.y + entity.collider.min.y >= y + 0.5f && entity.position.y + entity.collider.max.y <= y + height - 0.5f;
	}
}
...
public class LevelGenerator
{
	...
	void createDoors(bool spawnStartingRoom, bool spawnBossRoom, Room startingRoom, Room exitRoom, out Vector2i entrancePosition, out Vector2i exitPosition)
	{
		...
		if (spawnStartingRoom)
		{
			entrancePosition = new Vector2i(startingRoom.x + startingRoom.width / 2, startingRoom.y + startingRoom.height / 2);
	...
	public void generateCaves(string seed, int floor, bool dark, bool spawnStartingRoom, bool spawnBossRoom, Level level, Level nextLevel, Level lastLevel, Door entrance)
	{
		...
		int width = spawnStartingRoom ? MathHelper.RandomInt(80, 120, random) : MathHelper.RandomInt(40, 120, random);
		int height = Math.Max((floor == 4 ? 4500 : 3200) / width, 40);
		...
		spawnTileObject((int x, int y, TileType tile, TileType left, TileType right, TileType down, TileType up) =>
		{
			if (tile == null && (left == null && right == null) && !objectFlags[x + y * width] && getRoom(x, y) != startingRoom)
			{
				...
				if ((distanceToEntrance > 8 || y < entrancePosition.y))
				{
