...
public class Player : Entity, Hittable, StatusEffectReceiver
{
	...
	public float coinCollectDistance = 2.0f;
	...
	public Player()
	{
		...
		collider = new FloatRect(-0.1f, 0, 0.2f, 0.4f);
		...
		shadow = new Sprite(shadowsTileset, 0, 0);
	...
	*/
	...
	public void throwItem(Item item, bool shortThrow = false, bool farThrow = false)
	{
		...
		{
			itemVelocity += new Vector2(direction, 1) * (shortThrow ? new Vector2(0.4f, MathHelper.RandomFloat(-0.2f, 0.2f)) : farThrow ? new Vector2(2, MathHelper.RandomFloat(0.14f, 0.16f)) : new Vector2(1, MathHelper.RandomFloat(-0.5f, 0.5f))) * 14;
		}
		ItemEntity obj = new ItemEntity(item, this, itemVelocity) { zVelocity = 5 };
		GameState.instance.level.addEntity(obj, position);
	}
	...
	public ItemEntity throwItem(Item item, Vector2 direction, float speed = 14, bool throws = true)
	{
		...
		Vector2 throwOrigin = position + direction.normalized;
		ItemEntity obj = new ItemEntity(item, throws ? this : null, itemVelocity) { zVelocity = 4 };
	...
	void updateMovement()
	{
		...
		if (isAlive && !isStunned)
		{
			...
			isDucked = InputManager.IsDown("Duck") && numOverlaysOpen == 0;
		...
		{
			velocity = delta.normalized * speed * currentSpeedModifier;
			isMoving = true;
		}
		else
		{
		...
		if (isAlive && numOverlaysOpen == 0)
		{
			...
			else if (Settings.game.aimMode == AimMode.Directional)
			{
				if (Input.cursorHasMoved)
				{
					...
					if (MathF.Abs(Input.cursorPosition.x - playerScreenPos.x) > maxCursorDistance * 16 * SoulKnight.instance.scale ||
						MathF.Abs(Input.cursorPosition.y - playerScreenPos.y) > maxCursorDistance * 16 * SoulKnight.instance.scale)
					{
						int x = Math.Clamp(Input.cursorPosition.x, (int)MathF.Round(playerScreenPos.x - maxCursorDistance * 16 * SoulKnight.instance.scale), (int)MathF.Round(playerScreenPos.x + maxCursorDistance * 16 * SoulKnight.instance.scale));
						int y = Math.Clamp(Input.cursorPosition.y, (int)MathF.Round(playerScreenPos.y - maxCursorDistance * 16 * SoulKnight.instance.scale), (int)MathF.Round(playerScreenPos.y + maxCursorDistance * 16 * SoulKnight.instance.scale));
		...
		if (!isClimbing)
		{
			...
			if (!isAlive || !InputManager.IsDown("Jump"))
			{
				...
				if (InputManager.IsReleased("Jump"))
					zVelocity = MathF.Min(zVelocity, 0);
			}
			zVelocity += gravityMultiplier * gravity * Time.deltaTime;
			zVelocity = MathF.Max(zVelocity, MAX_FALL_SPEED);
	...
	public Vector3 getWeaponOrigin(bool mainHand)
	{
		...
		if (isDucked)
			animOffset.y = -3;
		return new Vector3((!mainHand ? 2 / 16.0f : -3 / 16.0f) + animOffset.x / 16.0f, (!mainHand ? 1 / 16.0f : -1 / 16.0f) + animOffset.y / 16.0f, 0.3f);
	}
	...
	void renderHandItem(float layer, bool mainHand, Item item)
	{
		...
		if (item.sprite != null)
		{
			...
			{
				Matrix weaponTransform = Matrix.CreateTranslation(position.x, position.y, 0.0f)
			...
			else
			{
				...
				{
					Vector2 weaponPosition = new Vector2(position.x + (item.renderOffset.x + getWeaponOrigin(mainHand).x) * direction, position.y + getWeaponOrigin(mainHand).y + getWeaponOrigin(mainHand).z);
					Renderer.DrawSprite(weaponPosition.x - 0.5f * item.size.x, weaponPosition.y - 0.5f * item.size.y, getWeaponOrigin(mainHand).z + 0.5f, item.size.x, item.size.y, 0, item.sprite, direction == -1, color);
					if (particles != null)
					{
	...
	public override void render()
	{
		...
		if (isVisible && show)
		{
			...
			Renderer.DrawSprite(snappedPosition.x - 0.5f, snappedPosition.y - 0.5f, 0.001f, 1, 1, 0, shadow, false, 0x7FFFFFFF);

			//Renderer.DrawSprite(snappedPosition.x - 0.5f, snappedPosition.y, 1, isDucked ? 0.5f : 1, sprite, direction == -1, 0xFFFFFFFF);
			Renderer.DrawVerticalSprite(snappedPosition.x - 0.5f, snappedPosition.y, 0, 1, isDucked ? 0.5f : 1, sprite, direction == -1, Vector4.One);
			...
			{
				if (passiveItems[i].ingameSprite != null)
					Renderer.DrawVerticalSprite(snappedPosition.x - 0.5f * passiveItems[i].ingameSpriteSize, snappedPosition.y, 0.001f, passiveItems[i].ingameSpriteSize, (isDucked ? 0.5f : 1) * passiveItems[i].ingameSpriteSize, passiveItems[i].ingameSprite, direction == -1, passiveItems[i].ingameSpriteColor);
