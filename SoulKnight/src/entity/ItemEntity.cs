...
public class ItemEntity : Entity, Interactable, Hittable
{
	...
	public override void render()
	{
		...
		Renderer.DrawVerticalSprite(position.x - 0.5f * item.size.x, position.y, z, item.size.x, item.size.y, item.sprite, flipped, rotation, color);
		//Renderer.DrawVerticalSprite(position.x - 0.5f * item.size.x, position.y, z, item.size.x, item.size.y, item.sprite, flipped, color);

		Player player = GameState.instance.player;
		if (outline != 0 && velocity.lengthSquared < 4 && player.velocity.lengthSquared < 4.0f)
		{
			Renderer.DrawVerticalOutline(position.x - 0.5f * item.size.x, position.y + 0.01f, z - 0.01f, item.size.x, item.size.y, rotation, item.sprite, flipped, outline);
