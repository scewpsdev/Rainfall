...
public class ParticleEffect : Entity
{
	...
	public override void init(Level level)
	{
		...
		for (int i = 0; i < systems.Length; i++)
			systems[i].setTransform(Matrix.CreateTranslation(position.x, z, position.y), true);
	}
	...
	public override unsafe void update()
	{
		...
		for (int i = 0; i < systems.Length; i++)
		{
			systems[i].setTransform(Matrix.CreateTranslation(position.x, z, position.y) * Matrix.CreateRotation(Vector3.UnitZ, rotation), true);
			systems[i].update();
		...
		for (int j = 0; j < systems.Length; j++)
		{
			...
			for (int i = 0; i < systems[j].numParticles; i++)
			{
				...
				if (particle.active)
				{
					...
					Renderer.DrawVerticalSprite(particle.position.x - 0.5f * size, particle.position.z, particle.position.y - 0.5f * size, size, size, particle.rotation, textureAtlases[j], u0, v0, w, h, particle.color, systems[j].handle->additive);
				}
			}
		}
	}
