...
public class AttackAction : EntityAction
{
	...
	{
		get => (stab ? currentProgress * attackRange : attackRange) * 2;
	}

	public float currentAngle
	{
		get => stab ? (new Vector2(MathF.Abs(direction.x), direction.y).angle) : (new Vector2(MathF.Abs(direction.x), direction.y).angle) + (swingIteration % 2 == 0 ? currentProgress : 1 - currentProgress) * 1.5f * MathF.PI - 0.75f * MathF.PI;
	}
