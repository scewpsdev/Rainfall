...
public class BowShootAction : EntityAction
{
	...
	public override void onFinished(Player player)
	{
		...
		Vector2 inaccuracy = Mathf.RandomPointOnCircle(Random.Shared) * 0.03f;
		direction = (direction + inaccuracy / (weapon.accuracy * player.getAccuracyModifier())).normalized;

		Vector2 position = player.center; // + new Vector2(player.direction * 0.25f, 0.0f); // player.position + Vector2.Up * 0.3f;
		Vector2 offset = Vector2.Zero; // new Vector2(player.direction * 0.25f, 0.0f /*0.45f*/);
