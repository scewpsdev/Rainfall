...
public class AttackAction : EntityAction
{
	...
	void checkHit(Vector2 origin, float progress, Player player)
	{
		...
		int numHits = GameState.instance.level.sweepNoBlock(origin, new FloatRect(-0.125f, -0.125f, 0.25f, 0.25f), direction, maxRange, hits, Entity.FILTER_MOB | Entity.FILTER_DEFAULT | Entity.FILTER_PROJECTILE);
		...
		if (lastTip != Vector2.Zero)
		{
			...
			hits = new HitData[16];
			numHits = GameState.instance.level.sweepNoBlock(lastTip, new FloatRect(-0.125f, -0.125f, 0.25f, 0.25f), fromLast.normalized, fromLast.length, hits, Entity.FILTER_MOB | Entity.FILTER_DEFAULT | Entity.FILTER_PROJECTILE);
			for (int i = 0; i < numHits; i++)
				processHit(hits[i], player);
		}
		...
		if (trail != null)
		{
			...
			if (anim == AttackAnim.SwingSideways)
				thickness = Mathf.Remap(Vector2.Dot(direction, Vector2.Rotate(Vector2.Right, currentAngle) * new Vector2(MathF.Sign(direction.x), 1)), -1, 1, 0.9f, 0.6f);
			else if (anim == AttackAnim.SwingOverhead)
				thickness = Mathf.Remap(Vector2.Dot(direction, Vector2.Rotate(Vector2.Right, currentAngle) * new Vector2(MathF.Sign(direction.x), 1)), -1, 1, 0.99f, 0.7f);
	...
	public override void update(Player player)
	{
		...
		if (inDamageWindow)
		{
			...
			{
				checkHit(origin, Mathf.Lerp(lastProgress, currentProgress, (i + 1) / (float)subSteps), player);
			}
	...
	public float getCurrentAngle(float progress)
	{
		...
		else
		{
			...
			float angle = Mathf.Lerp(new Vector2(MathF.Abs(direction.x), direction.y).angle + startAngle, new Vector2(MathF.Abs(direction.x), direction.y).angle + endAngle, progress);
			return angle;
		}
	}
