...
public class SpellCastAction : EntityAction
{
	...
	void cast(Player player)
	{
		if (spell.cast(player, weapon, manaCost, duration))
		{
...

			{
				...
				player.level.addEntity(ParticleEffects.CreateCastEffect(), position + offset);
			}
		}
	}
	...
	{
		if (spell.needsCharging && !cancelled)
		{
			cast(player);
		}
	}
	...
	public override void update(Player player)
	{
		...
		if ((player.currentAttackInput == null || !player.currentAttackInput.isDown()) && spell.needsCharging)
		{
			cancel();
			cancelled = true;
		}
	...
	public override Matrix getItemTransform(Player player, bool mainHand)
	{
		...
		else
		{
			...
			* Matrix.CreateRotation(Vector3.UnitZ, (direction * new Vector2(charDirection, 1)).angle + Mathf.Lerp(MathF.PI, 0, progress));
		}
	}
}
