...
public class Jetpack : Item
{
	...
	public override void update(Entity entity)
	{
		if (entity is Player)
		{
			...
			soundVolume = Mathf.Lerp(soundVolume, active ? 1 : 0, 5 * Time.deltaTime);
			...
			if (inputDown)
			{
				if (!active)
				{
					if (player.velocity.y < -14 || player.velocity.y < 0 && InputManager.IsPressed("Jump"))
						activate(player);
				}
				if (active)
				{
					if (!player.isStunned)
					{
						float powerMultiplier = 1 - MathF.Pow(Mathf.Clamp(player.velocity.y / maxVelocity, 0, 1), 2);
						player.velocity.y += powerMultiplier * power * Time.deltaTime;
						...
						for (int i = 0; i < numHits; i++)
						{
							if (hits[i].entity != null && hits[i].entity != player)
							{
								...
								if (hits[i].entity is Hittable)
								{
									...
									float damage = 10 * MathF.Pow(Mathf.Clamp(Mathf.Remap(distance, 0, hitboxDistance, 1, 0.1f), 0.1f, 1), 2);
