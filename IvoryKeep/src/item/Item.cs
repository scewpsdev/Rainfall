...
public abstract class Item
{
	...
	public static readonly Sound[] woodBreak = [Resource.GetSound("sounds/break_wood.ogg")];
	...
	{
		int idx = (int)MathF.Ceiling(Mathf.Remap(scaling, 0, 1, 0, scalingLetters.Length - 1));
	...
	public virtual int getValue() => baseValue;
	...
	public AttackAnim anim = AttackAnim.SwingOverhead;
	public float attackAcceleration = 1;
	...
	*/
	...
	public static Item CreateRandom(List<Item> items, Random random, float meanValue)
	{
		...
		for (int i = 0; i < items.Count; i++)
		{
			float gaussian = items[i].getValue() <= meanValue ?
				Mathf.Gaussian(items[i].getValue() / meanValue - 1, 1, 1.0f) :
				Mathf.Gaussian((items[i].getValue() - meanValue) / 8, 0, 1);
			gaussian /= Mathf.inv_sqrt_2pi;
		...
		while (newItem.stackable && newItem.type != ItemType.Food && newItem.getValue() * newItem.stackSize * 3 / 2 < 0.5f * meanValue)
		{
			int difference = (int)(meanValue / newItem.getValue() - newItem.stackSize);
		...
		foreach (int idx in list)
		{
			...
			if (f < cumulativeRarity)
			{
				...
				if (newItem.name == "arrow")
					newItem.stackSize = Mathf.RandomInt(1, 35, random);
				else if (newItem.name == "throwing_knife")
					newItem.stackSize = Mathf.RandomInt(1, 10, random);
				if (newItem.type == ItemType.Staff)
					newItem.staffCharges = Mathf.RandomInt(newItem.maxStaffCharges / 2, newItem.maxStaffCharges, random);
				return newItem;
			}
		}
	...
	public static Item[] CreateRandom(Random random, float[] distribution, float meanValue)
	{
		...
		if (type != ItemType.Count)
		{
			...
			if (item.requiredAmmo != null)
			{
				...
				ammo.stackSize = Mathf.RandomInt(3, 20, random);
				return [item, ammo];
			}
			else
			{
