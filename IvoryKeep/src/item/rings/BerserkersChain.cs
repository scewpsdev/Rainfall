...
public class BerserkersChain : Item
{
		...
		: base("berserkers_chain", ItemType.Relic)
	{
		...
		buff.auraColor = 0xFFd16b6b; // 0xFFd82b2b;
	}
	...
	public override void onKill(Player player, Mob mob)
	{
		...
		int buffLevelIncrease = 1;
		buffLevel = Math.Min(buffLevel + buffLevelIncrease, (2 - 1) * 20 + threshhold);
	...
	public override void update(Entity entity)
	{
		if (entity is Player)
		{
			...
			if ((Time.currentTime - lastTick) / 1e9f > 5)
			{
				...
				{
					int lastBuffLevel = buffLevel;
					int cooldown = (int)Math.Ceiling((Time.currentTime - lastKill) / 1e9f / 8);
