		...
		for (int j = 0; j < systems.Length; j++)
		{
			...
			for (int i = 0; i < systems[j].numParticles; i++)
			{
				...
				if (particle.active)
				{
					...
					Renderer.DrawUISprite((int)(particle.position.x * 16), (int)(Renderer.UIHeight - particle.position.y * 16 - 1), (int)size, (int)size, textureAtlases[j], u0, v0, w, h, Mathf.VectorToARGB(particle.color));
				}
			}
		}
	}
