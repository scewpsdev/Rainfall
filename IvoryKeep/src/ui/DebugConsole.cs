	...
	static string RunCommand(string cmd, string[] args)
	{
		...
		else if (cmd == "create_entity")
		{
			if (args.Length > 0)
			{
				...
				if (entity != null)
				{
					Vector2 position = GameState.instance.player.position;
					...
					if (entity is not Mob)
						position += GameState.instance.player.collider.center; //GameSettings.aimMode == AimMode.Directional ? GameState.instance.player.position + GameState.instance.player.collider.center + GameState.instance.player.lookDirection : GameState.instance.camera.screenToWorld(Renderer.cursorPosition);
		...
		if (cmd == "reset_quest")
		{
			if (args.Length == 2)
			{
				if (QuestManager.getQuestList(GameState.instance.save, args[0], out List<Quest> questList))
				{
