...
public class InventoryUI
{
	...
	static InventoryUI()
	{
		...
		tabIcons = [weaponSprite, armorSprite, bagSprite, spellSprite, bagSprite, ringSprite];
	}
	...
	{
		ItemSlotUI.Render(x, y, size, item?.icon, item != null ? Mathf.VectorToARGB(item.spriteColor) : 0xFFFFFFFF, item != null ? item.stackSize : 1, background, cellPosition, ref selectedCell);
	...
	*/
	...
	public static int DrawEquipment3(int x, int y, int width, int height, Player player, ref Vector2i selectedCell, out Item item)
	{
		...
		Vector2i firstPassiveItem = firstSpellItem + new Vector2i(0, (player.spellCapacity + 3) / 4);
		...
		if (selectedCell.y >= firstSpellItem.y && selectedCell.y < /*firstStoredItem.y*/ firstPassiveItem.y)
		{
	...
	List<Item> weaponsCache = new List<Item>();
	List<Item> armorCache = new List<Item>();
	List<Item> consumablesCache = new List<Item>();
	List<Item> spellsCache = new List<Item>();
	List<Item> utilsCache = new List<Item>();
	List<Item> ringsCache = new List<Item>();
	...
	public void render()
	{
		...
		if (player.inventoryOpen)
		{
			...
			if (InputManager.IsPressed("UILeft", true) || Input.IsKeyPressed(KeyCode.J, true))
			{
				currentItemTab = (currentItemTab + (int)InventoryTab.Count - 1) % (int)InventoryTab.Count;
				selectedItemIdx = 0;
			}
			if (InputManager.IsPressed("UIRight", true) || Input.IsKeyPressed(KeyCode.L, true))
			{
				currentItemTab = (currentItemTab + 1) % (int)InventoryTab.Count;
				selectedItemIdx = 0;
			}

			weaponsCache.Clear();
			armorCache.Clear();
			consumablesCache.Clear();
			spellsCache.Clear();
			utilsCache.Clear();
			ringsCache.Clear();
			...
			{
				if (item.type == ItemType.Weapon || item.type == ItemType.Shield || item.type == ItemType.Staff)
					weaponsCache.Add(item);
				else if (item.type == ItemType.Armor)
					armorCache.Add(item);
				else if (/*item.type == ItemType.Spell ||/ item.type == ItemType.Scroll)
					spellsCache.Add(item);
				else if (item.type == ItemType.Food || item.type == ItemType.Potion || item.type == ItemType.Utility)
					consumablesCache.Add(item);
				else if (item.type == ItemType.Relic || item.type == ItemType.Gem)
					ringsCache.Add(item);
			...
			List<Item>[] tabs = [weaponsCache, armorCache, consumablesCache, spellsCache, utilsCache, ringsCache];
			List<Item> currentTab = tabs[currentItemTab];

			//inventoryHeight = DrawEquipment3(x, y, width, inventoryHeight, player, ref selectedCell, out Item selected);
			int choice = ItemSelector.Render(x, y, "Inventory", currentTab, null, player.money, player, true, null, false, out bool secondary, out bool closed, ref selectedItemIdx, (float x, float y, int width) =>
			{
				const int height = 16;

				Renderer.DrawUISprite(x - 1, y - 1, width + 2, height + 1, null, false, UIColors.WINDOW_FRAME);
				Renderer.DrawUISprite(x, y, width, height, null, false, UIColors.WINDOW_BACKGROUND);
				for (int i = 0; i < tabs.Length; i++)
				{
					Renderer.DrawUISprite(x + i * 16, y, 16, 16, null, false, currentItemTab == i ? UIColors.ITEM_SLOT_FRAME_HIGHLIGHT : UIColors.ITEM_SLOT_FRAME);
					Renderer.DrawUISprite(x + i * 16 + 1, y + 1, 14, 14, null, false, currentItemTab == i ? UIColors.ITEM_SLOT_BACKGROUND_HIGHLIGHT : UIColors.ITEM_SLOT_BACKGROUND);
					Renderer.DrawUISprite(x + i * 16, y, 16, 16, tabIcons[i]);
				}
				return height;
			});
			Item selected = currentTab.Count > 0 ? currentTab[selectedItemIdx] : null;
			*/
			if (selected != null)
			{
