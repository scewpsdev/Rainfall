	...
	public static int Render(string[] labels, bool[] enabled, ref int currentButton, float[] fade = null)
	{
		...
		for (int i = 0; i < labels.Length; i++)
		{
			...
			if (enabled != null && !enabled[i])
				...
				color = Mathf.VectorToARGB(Vector4.Lerp(0xFFFFFFFF, 0xFF444444, MathF.Sin(Time.timestamp / 1e9f * 3) * 0.5f + 0.5f));
			uint bgColor = 0xFF444444;

			if (fade != null)
			{
				color = Mathf.ColorAlpha(color, fade[i]);
				bgColor = Mathf.ColorAlpha(bgColor, fade[i]);
			}

			if (selected || enabled != null && !enabled[i])
			{
