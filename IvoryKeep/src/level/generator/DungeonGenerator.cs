...
public partial class LevelGenerator
{
	...
	public void generateDungeons(string seed, out Level[] areaDungeons)
	{
		...
		Vector3 ambience = Mathf.ARGBToVector(0xFF3b3159).xyz;
		areaDungeons[0] = new Level(9, "dungeons1", "Weeping Catacombs", 60, 40, null, 40) { ambientLight = ambience };
		areaDungeons[1] = new Level(10, "dungeons2", "", 40, 50, null, 44) { ambientLight = ambience };
		areaDungeons[2] = new Level(11, "dungeons3", "", 50, 50, null, 48) { ambientLight = ambience };
		areaDungeons[3] = new Level(12, "dungeons4", "", 40, 60, TileType.bricks, 52) { ambientLight = ambience };
		areaDungeons[4] = new Level(-1, "dungeons_boss_room", "Forgotten Chamber", 40, 20, TileType.bricks, 52) { ambientLight = ambience }; // loot value will affect what the blacksmith sells in the hub
	...
	void generateDungeonFloor(string seed, int floor, bool spawnStartingRoom, bool spawnBossRoom, Level level, Level nextLevel, Level lastLevel, Door entrance, Func<List<Mob>> createEnemy)
	{
		...
		generateMainRooms(dungeonsSet, startingRoomDef, spawnBossRoom);
		if (spawnBossRoom)
			rooms.Reverse();
