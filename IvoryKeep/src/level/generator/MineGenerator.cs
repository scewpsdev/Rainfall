...
public partial class LevelGenerator
{
	...
	public void generateMines(string seed, out Level[] areaMines)
	{
		...
		areaMines[0] = new Level(5, "mines1", "Crystal Mines", 24, 70, TileType.dirt, 24) { ambientLight = mediumAmbience };
		areaMines[1] = new Level(6, "mines2", "", 70, 30, TileType.dirt, 28) { ambientLight = mediumAmbience };
		areaMines[2] = new Level(7, "mines3", "", 40, 40, TileType.dirt, 32) { ambientLight = mediumAmbience };
		areaMines[3] = new Level(8, "mines4", "", 30, 80, TileType.dirt, 36) { ambientLight = mediumAmbience };
		areaMines[4] = new Level(-1, "mines_boss_room", "", 36) { ambientLight = mediumAmbience }; // loot value will affect what the blacksmith sells in the hub
	...
	{
		Room room = generateSingleRoomLevel(level, specialSet, 5, TileType.stone, TileType.rock);
	...
	void generateMinesFloor(string seed, int floor, bool spawnStartingRoom, bool spawnBossRoom, Level level, Level nextLevel, Level lastLevel, Door lastExit, Func<List<Mob>> createEnemy)
	{
		...
		level.ambientTrack = minesAmbientTrack;
		...
		generateMainRooms(minesSet, startingRoomDef, spawnBossRoom);
		if (spawnBossRoom)
			rooms.Reverse();
		...
		*/
		...
		spawnRoomObject(roomsWithStartingWeapon, 1, false, (Vector2i tile, Random random, Room room) =>
		{
			...
			if (item.requiredAmmo != null)
			{
				...
				ammo.stackSize = Mathf.RandomInt(20, 36, random);
				items = [item, ammo];
			}
			else
			{
