...
public partial class LevelGenerator
{
	...
	public void generateGardens(string seed, out Level[] areaGardens)
	{
		...
		areaGardens[0] = new Level(13, "gardens1", "Royal Gardens", 80, 80, TileType.dirt, 56);
		areaGardens[1] = new Level(14, "gardens2", "", 100, 40, TileType.dirt, 60);
		areaGardens[2] = new Level(15, "gardens3", "", 40, 100, TileType.dirt, 64);
		areaGardens[3] = new Level(-1, "gardens_boss_room", "", 64);
	...
	void generateGardenFloor(string seed, int floor, bool spawnStartingRoom, bool spawnBossRoom, Level level, Level nextLevel, Level lastLevel, Door entrance, Func<List<Mob>> createEnemy)
	{
		...
		level.rooms = rooms;
		level.ambientLight = Mathf.ARGBToVector(0xFFdcffb5).xyz * 0.3f;
		...
		generateMainRooms(gardensSet, startingRoomDef, spawnBossRoom);
		if (spawnBossRoom)
			rooms.Reverse();
		...
		{
			int amount = Mathf.RandomInt(2, 10, random);
