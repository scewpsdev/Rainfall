...
public class Level
{
	...
	*/
	...
	public Level(int floor, string name, string displayName, float avgLootValue = 0)
	{
		...
		this.avgLootValue = avgLootValue;
	...
	public Level(int floor, string name, string displayName, int width, int height, TileType defaultTile, float avgLootValue = 0)
		: this(floor, name, displayName, avgLootValue)
	{
	...
	public unsafe void updateLightmap(int x0, int y0, int w, int h)
	{
		...
		for (int y = y0; y <= y1; y++)
		{
			for (int x = x0; x <= x1; x++)
			{
				TileType t0 = getTile(Math.Max(x - 1, 0), Math.Max(y - 1, 0));
				TileType t1 = getTile(Math.Min(x, width - 1), Math.Max(y - 1, 0));
				TileType t2 = getTile(Math.Max(x - 1, 0), Math.Min(y, height - 1));
				TileType t3 = getTile(Math.Min(x, width - 1), Math.Min(y, height - 1));
				byte value = 0;
				if ((t0 == null || !t0.isSolid || !t0.visible)) value += 64;
				if ((t1 == null || !t1.isSolid || !t1.visible)) value += 64;
				if ((t2 == null || !t2.isSolid || !t2.visible)) value += 64;
	...
	bool overlapSolid(Vector2 min, Vector2 max, bool falling, bool downInput, FloatRect ignoreCollider, uint filterMask)
	{
		...
		{
			if (colliders[i].collider != ignoreCollider && (colliders[i].filterGroup & filterMask) != 0)
			{
	...
	public int doCollision(ref Vector2 position, FloatRect collider, ref Vector2 displacement, bool downInput = false, bool cornerCutting = false, uint filterMask = 1)
	{
		...
		if (flags == 0 && overlapSolid(position + collider.min + displacement, position + collider.max + displacement, displacement.y < 0, downInput, collider, filterMask))
		{
			float displacementFactor = 0.5f;
			for (int i = 1; i < 10; i++)
			{
				if (overlapSolid(position + collider.min + displacement * displacementFactor, position + collider.max + displacement * displacementFactor, displacement.y < 0, downInput, collider, filterMask))
					displacementFactor -= MathF.Pow(0.5f, i + 1);
				else
				{
					if (overlapSolid(position + collider.min + new Vector2(displacement.x, displacement.y * displacementFactor), position + collider.max + new Vector2(displacement.x, displacement.y * displacementFactor), displacement.y < 0, downInput, collider, filterMask))
					{
	...
	public HitData raycastSolid(Vector2 origin, Vector2 direction, float range, uint filterMask = 1)
	{
		...
		for (int i = 0; i < 128; i++)
		{
			...
			mm = new Vector2i(Mathf.Step(dis.x, dis.y), Mathf.Step(dis.y, dis.x));
			dis += mm * rs * ri;
			pos += mm * rs;
		}
		...
		for (int i = 0; i < colliders.Count; i++)
		{
			if (!colliders[i].platformCollider && (colliders[i].filterGroup & filterMask) != 0)
			{
	...
	public HitData hitTiles(Vector2 position, uint filterMask = 1)
	{
		...
		for (int i = 0; i < colliders.Count; i++)
		{
			if (!colliders[i].platformCollider && (filterMask & colliders[i].filterGroup) != 0)
			{
				Vector2 min = colliders[i].position + colliders[i].collider.min;
				Vector2 max = colliders[i].position + colliders[i].collider.max;
				if (position.x >= min.x && position.x <= max.x && position.y >= min.y && position.y <= max.y)
