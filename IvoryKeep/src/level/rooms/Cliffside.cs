...
public class Cliffside : Entity
{
	...
	public override void render()
	{
		...
		for (int i = 0/*-5*/; i < waveLayers; i++)
		{
			...
			//Vector4 color = new Vector4(Mathf.SRGBToLinear(Mathf.ARGBToVector(0xFF36b3be).xyz * brightness * 1.2f) + Mathf.ARGBToVector(0xFF6eafeb).xyz * 0.3f, 1);
			Vector4 color = Mathf.SRGBToLinear(Mathf.ARGBToVector(0xFF36b3be) * brightness * 1.2f);
