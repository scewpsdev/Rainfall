...
public class StartingClass
{
	...
	static StartingClass()
	{
		...
		thief = new StartingClass() { name = "Thief", color = 0xFF676767, cost = 8, items = [new Dagger(), new DarkHood(), new DarkCloak(), (new GreenPotion() { stackSize = 3 }).makeThrowable(), new ThrowingKnife() { stackSize = 5 }, new Parsley(), new Nightstalker()] };
		wizard = new StartingClass() { name = "Wizard", color = 0xFF73549d, cost = 8, items = [new MagicArrowStaff(), /*new MagicArrowSpell(),*/ new WizardHat(), new WizardsCloak(), new WizardsLegacy()], magic = 6 };
		fool = new StartingClass() { name = "Fool", color = 0xFFc89d3b, cost = 1, items = [new Club(), new GlassRing()], hp = 6 };
