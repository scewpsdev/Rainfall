...
public static class Renderer
{
	...
	static Texture luminanceReadbackTexture;
	static uint[] luminanceDataBuffer;
	static GCHandle luminanceDataBufferHandle;
	static float targetExposure = 1;
	static float currentExposure = 1;
	static uint nextLuminanceReadbackFrame = uint.MaxValue;
	...
	public static void Init(GraphicsDevice graphics, int width, int height)
	{
		...
		bloomStepCount = 0;
		int s = Math.Max(Display.width, Display.height);
		while (true)
		{
			s >>= 1;
			if (s == 0)
		...
		for (int i = 0; i < bloomStepCount; i++)
		{
			...
			int w = Math.Max(Display.width / exp, 1);
			int h = Math.Max(Display.height / exp, 1);
		...
		luminanceReadbackTexture = graphics.createTexture(16, 16, TextureFormat.RG11B10F, (ulong)TextureFlags.BlitDst | (ulong)TextureFlags.ReadBack);
		luminanceDataBuffer = new uint[16 * 16];
	...
	public static void DrawSprite(float x, float y, float z, float width, float height, float rotation, Sprite sprite, bool flipX, bool flipY, Vector4 color, bool additive)
	{
		...
		if (sprite != null)
		{
			...
			if (flipX)
				Mathf.Swap(ref u0, ref u1);
			if (flipY)
				Mathf.Swap(ref v0, ref v1);
		}
	...
	public static void DrawSprite(float width, float height, Matrix transform, Sprite sprite, bool flipped, Vector4 color)
	{
		...
		DrawSprite(width, height, transform, sprite.spriteSheet.texture, u0, v0, w, h, color);
	}
	...
	*/
	...
	{
		DrawVerticalSprite(x, y, 0.0f, width, height, sprite, flipped, Mathf.ARGBToVector(color));
	}
	...
	public static void DrawVerticalSprite(float x, float y, float z, float width, float height, Texture texture, int u0, int v0, int w, int h, uint color = 0xFFFFFFFF)
	{
		...
		verticalDraws.Add(new SpriteDraw { position = new Vector3(x, y, z), size = new Vector2(width, height), texture = texture, rect = rect, color = Mathf.ARGBToVector(color) });
	}
	*/
	...
	{
		FloatRect rect = new FloatRect(u0 / (float)texture.width, v0 / (float)texture.height, w / (float)texture.width, h / (float)texture.height);
		verticalDraws.Add(new SpriteDraw { position = new Vector3(x, y, z), size = new Vector2(width, height), texture = texture, rect = rect, rotation = rotation, color = Mathf.ARGBToVector(color) });
	}
	...
	public static void DrawWorldTextBMP(float x, float y, float z, string text, float scale, uint color = 0xFFFFFFFF, bool additive = false)
	{
		float cursor = 0;
		for (int i = 0; i < text.Length; i++)
		{
	...
	{
		graphics.resetState();
		graphics.setPass((int)RenderPass.Bloom + 15 + idx);
	...
	static void PostProcessingPass()
	{
		if (bloomEnabled)
		{
			for (int i = 0; i < bloomStepCount; i++)
			{
				Texture input = i == 0 ? composite.getAttachmentTexture(0) : bloomDownsampleChain[i - 1].getAttachmentTexture(0);
				RenderTarget output = bloomDownsampleChain[i];
				BloomDownsample(i, input, output);
			}

			for (int i = 0; i < bloomStepCount - 1; i++)
			{
				BloomUpsample(i, bloomDownsampleChain[bloomStepCount - 1 - i].getAttachmentTexture(0), bloomDownsampleChain[bloomStepCount - 2 - i].getAttachmentTexture(0), bloomUpsampleChain[bloomStepCount - 2 - i]);
			}
		}
		else
		{
	...
	static Vector3 DecodeRG11B10(uint bits)
	{
		uint rb = (bits & 0b11111111111);
		uint gb = (bits & 0b1111111111100000000000) >> 11;
		uint bb = (bits & 0b11111111110000000000000000000000) >> 22;
		...
		uint expr = (rb & 0b11111000000) >> 6;
		uint expg = (gb & 0b11111000000) >> 6;
		uint expb = (bb & 0b1111100000) >> 5;
		uint manr = rb & 0b111111;
		uint mang = gb & 0b111111;
		uint manb = bb & 0b11111;
		float r = MathF.Pow(2, (float)expr - 15) * (1.0f + manr / (float)0b1000000);
		float g = MathF.Pow(2, (float)expg - 15) * (1.0f + mang / (float)0b1000000);
		float b = MathF.Pow(2, (float)expb - 15) * (1.0f + manb / (float)0b100000);
	...
	static void BlitPass()
	{
		...
		graphics.blit(luminanceReadbackTexture, 0, 0, 0, 0, bloomDownsampleChain[bloomDownsampleChain.Length - 1].getAttachmentTexture(0), 0, 0, 0, 0, 1, 1, 0);
		if (nextLuminanceReadbackFrame == uint.MaxValue)
		{
			luminanceDataBufferHandle = GCHandle.Alloc(luminanceDataBuffer, GCHandleType.Pinned);
			nextLuminanceReadbackFrame = graphics.readTexture(luminanceReadbackTexture, luminanceDataBufferHandle);
		}
		else if (graphics.getFrameIndex() >= nextLuminanceReadbackFrame)
		{
			Vector3 color = DecodeRG11B10(luminanceDataBuffer[0]);
			float luminance = color.x * 0.3f + color.y * 0.59f + color.z * 0.11f;
			targetExposure = MathF.Pow(1.0f / luminance * 0.1f, 1.0f / 4) * 0.5f;
			nextLuminanceReadbackFrame = uint.MaxValue;
			luminanceDataBufferHandle.Free();
		}

		float adaptionSpeed = currentExposure < targetExposure ? 1 : 2;
		currentExposure = Mathf.Lerp(currentExposure, targetExposure, adaptionSpeed * Time.deltaTime);
		...
		graphics.setUniform(blitShader, "u_bloomSettings", new Vector4(bloomStrength, bloomFalloff, currentExposure, 0));
	...
	static void UIPass()
	{
		...
		for (int i = 0; i < uiDraws.Count; i++)
		{
			...
			uiBatch.draw(
				...
				u0, v0, u1, v1,
				Mathf.ARGBToVector(draw.color), draw.solid ? 0.0f : 1.0f);
		}
		uiBatch.end();
