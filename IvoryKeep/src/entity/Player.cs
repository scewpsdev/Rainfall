...
public class Player : Entity, Hittable, StatusEffectReceiver, CoinTarget
{
	...
	public const int defaultItemCapacity = 3;
	public Item[] activeItems = new Item[defaultItemCapacity];
	...
	public override void onLevelSwitch(Level newLevel)
	{
		...
		for (int i = 0; i < items.Count; i++)
			items[i].onLevelSwitch(newLevel);
	}
	...
	public bool hasItemEquipped<T>() where T : Item
	{
		if (handItem != null && handItem.GetType() == typeof(T) ||
			offhandItem != null && offhandItem.GetType() == typeof(T))
			return true;
		for (int i = 0; i < passiveItems.Count; i++)
		{
			if (passiveItems[i].GetType() == typeof(T))
				return true;
		}
		return false;
	}
	...
	public void throwItem(Item item, bool shortThrow = false, bool farThrow = false)
	{
		...
		if (inputUp)
		{
			itemVelocity += new Vector2(direction * 0.05f, Mathf.RandomFloat(0.9f, 1.1f)).normalized * 14;
		}
		else if (inputDown)
		{
			itemVelocity += new Vector2(direction * 0.05f, Mathf.RandomFloat(-1.1f, -0.9f)) * 14;
			if (!isGrounded)
				velocity.y = MathF.Max(velocity.y, jumpPower);
		}
		else
		{
			itemVelocity += new Vector2(direction, 1) * (shortThrow ? new Vector2(0.4f, 0.1f) : farThrow ? new Vector2(2, 0.1f) : new Vector2(1, 0.1f)) * 14;
		}
	...
	public ItemEntity throwItem(Item item, Vector2 direction, float speed = 14, bool throws = true)
	{
		...
		Vector2 throwOrigin = center + direction.normalized * 0.1f;
		ItemEntity obj = new ItemEntity(item, throws ? this : null, itemVelocity);
		if (item.projectileSpins)
			obj.rotationVelocity = MathF.PI * Mathf.RandomFloat(-5, 5);
		GameState.instance.level.addEntity(obj, throwOrigin);
	...
	public void throwObject()
	{
		...
		carriedObject.velocity = new Vector2(0.5f, 1) * velocity + (lookDirection.normalized + Vector2.Up * 0.2f) * 10;
		carriedObject.rotationVelocity = Mathf.RandomFloat(-1, 1) * 10;
	...
	public bool hit(float damage, Entity by = null, Item item = null, string byName = null, bool triggerInvincibility = true, bool buffedHit = false)
	{
		...
		else if (blockingItem != null && (((BlockAction)actions.currentAction).isBlocking || ((BlockAction)actions.currentAction).isParrying))
		{
			...
			if (blockAction.isParrying && mob != null && mob.ai is AdvancedAI && (mob.ai as AdvancedAI).currentAction != null)
			{
				damage = 0;
		...
		if (by != null)
		{
			if (triggerInvincibility)
				GameState.instance.level.addEntity(ParticleEffects.CreateBloodEffect((position - by.center).normalized), position + collider.center);
		}
		else
		{
			if (triggerInvincibility)
	...
	void onDeath(Entity by, string byName)
	{
		if (handItem != null)
		{
			...
			Vector2 direction = new Vector2(0, 1) + Mathf.RandomVector2(-0.5f, 0.5f);
	...
	void updateMovement()
	{
		...
		inputLeft = InputManager.IsDown("Left");
		inputRight = InputManager.IsDown("Right");
		inputUp = InputManager.IsDown("Up");
		inputDown = InputManager.IsDown("Down");
		inputSprint = InputManager.IsDown("Sprint");
		...
		if (isAlive && !isStunned && (actions.currentAction == null || actions.currentAction.canMove))
		{
			...
			if (canWallJump)
			{
				if (InputManager.IsDown("Right") && GameState.instance.level.overlapSolid(position + new Vector2(0, 0.1f), position + new Vector2(collider.max.x + 0.2f, collider.max.y - 0.05f)))
				{
			...
			isLookingUp = isGrounded && inputUp;
			...
			if (InputManager.IsPressed("Jump") && (actions.currentAction == null || actions.currentAction.canJump))
			{
				if (isClimbing)
				{
					velocity.y = (inputDown ? -0.5f : 1) * jumpPower * getJumpPowerModifier();
					lastJumpInput = 0;
					lastGrounded = 0;
					lastLadderJumpedFrom = currentLadder;
					currentLadder = null;
				...
				else
				{
					...
					{
						velocity.y = jumpPower * getJumpPowerModifier();
					...
					else if (!isGrounded)
					{
						...
						{
							velocity.y = jumpPower * getJumpPowerModifier(); // * 0.75f;
		...
		{
			targetVelocity = delta.x * speed * currentSpeedModifier;
			if (actions.currentAction != null && actions.currentAction.actionMovement != 0)
				targetVelocity += actions.currentAction.actionMovement;
		}

		bool slowControl = isSprinting; // MathF.Abs(velocity.x) > 1.1f * defaultSpeed * equipLoadModifier * getMovementSpeedModifier() * (carriedObject != null ? 0.7f : 1);
		if (slowControl)
			velocity.x = Mathf.Lerp(velocity.x, targetVelocity, 1 - MathF.Pow(0.05f, Time.deltaTime));
		else
		...
		if (Renderer.cursorMove != Vector2i.Zero && numOverlaysOpen == 0)
		{
			...
			aimPosition.x = Mathf.Clamp(aimPosition.x, -20, 20);
			aimPosition.y = Mathf.Clamp(aimPosition.y, -20, 20);
			...
			float angle = Mathf.LerpAngle(currentAngle, dstAngle, 50 * Time.deltaTime);
		...
		*/
		...
		if (isAlive && numOverlaysOpen == 0)
		{
			if (Settings.game.aimMode == AimMode.Simple)
			{
				...
				else if (!isGrounded && inputDown)
					lookDirection = Vector2.Down;
				else
					lookDirection = new Vector2(direction, 0);
				lookDirection.x += MathF.Sign(delta.x);
				lookDirection = lookDirection = lookDirection.normalized;
			}
		...
		isGrounded = GameState.instance.level.overlapSolid(position + collider.min + new Vector2(0, -0.1f), position + new Vector2(collider.max.x, collider.min.y + 0.1f));
	...
	void updateActions()
	{
		if (isAlive && GameState.instance.run.active)
		{
			...
			if (!isStunned && numOverlaysOpen == 0)
			{
				...
				else if (isDucked && (carriedObject != null || handItem != null) && InputManager.IsPressed("Interact", true))
				{
	...
	void updateStatus()
	{
		if (isSprinting && isMoving)
			...
			mana = Mathf.Clamp(mana + defaultManaRecoveryRate * maxMana * getManaRecoveryModifier() * Time.deltaTime, 0, maxMana);
	...
	unsafe void updateAnimation()
	{
		...
		if (lastWallTouchLeft == Time.currentTime && InputManager.IsDown("Left") || lastWallTouchRight == Time.currentTime && InputManager.IsDown("Right"))
		{
			...
			{
				wallSlideParticles.systems[0].handle->colorAnim.value0.value.xyz = Mathf.ARGBToVector(tile.particleColor).xyz;
				wallSlideParticles.systems[0].handle->colorAnim.value1.value.xyz = Mathf.ARGBToVector(tile.particleColor).xyz;
				wallSlideParticles.systems[0].handle->colorAnim.value2.value.xyz = Mathf.ARGBToVector(tile.particleColor).xyz;
	...
	void onStep()
	{
		...
		if (!isDucked)
		{
			...
			if (tile != null)
				GameState.instance.level.addEntity(ParticleEffects.CreateStepEffect(Mathf.RandomInt(2, 4), 0.333f, Mathf.ARGBToVector(tile.particleColor).xyz, this), position);
			...
			if (boots.stepSound != null)
				Audio.PlayOrganic(boots.stepSound, new Vector3(position, 0));
		}
	}
	...
	void onLand()
	{
		if (velocity.y < -10)
		{
			TileType tile = GameState.instance.level.getTile(position - new Vector2(0, 0.5f));
			if (tile != null)
				GameState.instance.level.addEntity(ParticleEffects.CreateStepEffect(Mathf.RandomInt(4, 8), 0, Mathf.ARGBToVector(tile.particleColor).xyz, this), position);
		}
	...
	public override void update()
	{
		...
		Audio.UpdateListener(new Vector3(position, 10), Quaternion.Identity);
		Audio.Set3DVolume(500.0f);
	...
	void renderHand(bool mainHand, float layer, Vector2 handPosition, float handRotation, bool flip)
	{
		...
		Vector4 handColor = 0xFF838383; // 0xFF282828;
		...
		if (item.sprite != null)
		{
			if (actions.currentAction != null && (actions.currentAction.getRenderWeapon(mainHand) != null || item.customAttackRender))
			{
				...
				if (renderWeapon != null)
				{
					...
					Renderer.DrawOutline(item.size.x, item.size.y, Matrix.CreateTranslation(position.x, position.y, layer) * weaponTransform, item.sprite, false, 0xFF000000);
			...
			else
			{
				if (item.ingameSprite != null && item != DefaultWeapon.instance)
				{
					...
					//Vector3 buffColor = Mathf.ARGBToVector(0xFFdac66c).xyz;
					//float buffIntensity = Mathf.Remap(MathF.Sin(Time.currentTime / 1e9f * 60), -1, 1, 0.1f, 1);
	...
	public float getJumpPowerModifier()
	{
		float value = 1;
		foreach (ItemBuff modifier in itemBuffs)
		{
	...
	public float getMeleeDamageModifier()
	{
		...
		value *= MathF.Pow(1.08f, strength - 1);
		return value;
	}
	...
	public float getRangedDamageModifier()
	{
		...
		value *= MathF.Pow(1.08f, dexterity - 1);
		return value;
	}
	...
	public float getMagicDamageModifier()
	{
		...
		value *= MathF.Pow(1.08f, intelligence - 1);
		return value;
	}
	...
	public float getProjectileRangeModifier()
	{
		float value = 1;
		foreach (ItemBuff modifier in itemBuffs)
		{
			if (modifier.active)
				value *= MathF.Pow(modifier.projectileRangeModifier, modifier.item.stackSize);
		}
		return value;
	}
