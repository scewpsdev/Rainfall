		...
		: base("raya")
	{
...

		{
			...
			jumpAttack.onFinished = (AIAction action) =>
			{
				...
				if (tile != null)
					GameState.instance.level.addEntity(ParticleEffects.CreateImpactEffect(Vector2.Up, 6, 16, Mathf.ARGBToVector(tile.particleColor).xyz), ai.mob.position + ai.mob.direction * Vector2.Right);
				Audio.Play(landSound, new Vector3(ai.mob.position, 0));
			};
		}
