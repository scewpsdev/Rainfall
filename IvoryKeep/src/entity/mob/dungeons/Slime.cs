...
public class Slime : Mob
{
		...
		: base("slime")
	{
		...
		health = (2 * size + 1) / 2;
		jumpPower = size * 4; //  Mathf.IPow(2, size) * 3 / 2;
		speed = 0.5f;
	...
	public override void onDeath(Entity by, Item item)
	{
		if (size > 2)
		{
			int subSlimes = 4; // Mathf.RandomInt(1, 4);
			for (int i = 0; i < subSlimes; i++)
			{
				Slime slime = new Slime(size - 2);
				slime.spriteColor = spriteColor;
				GameState.instance.level.addEntity(slime, position + new Vector2(Mathf.RandomFloat(collider.min.x - slime.collider.min.x, collider.max.x - slime.collider.max.x), Mathf.RandomFloat(collider.min.y - slime.collider.min.y, collider.max.y - slime.collider.max.y)));
			}
		}
	...
	public override void onDeath(Entity by)
	{
		...
		if (size > 2)
		{
			int subSlimes = Mathf.RandomInt(1, 4);
			for (int i = 0; i < subSlimes; i++)
			{
				Slime slime = new Slime(size - 2);
				slime.spriteColor = spriteColor;
				GameState.instance.level.addEntity(slime, position + new Vector2(Mathf.RandomFloat(-0.2f, 0.2f), 0.1f));
			}
		}
	}
	*/
