...
public class LeprechaunAI : AdvancedAI
{
	...
	public override void onAttack(Entity e)
	{
		if (e is Player)
		{
			...
			if (player.money > 0)
			{
				int stealAmount = Mathf.RandomInt(1, (int)MathF.Ceiling(player.money * MathF.Exp(-player.money * 0.004f)));
				player.money -= stealAmount;
				while (stealAmount > 0)
				{
					CoinType value = Coin.SubtractCoinFromValue(ref stealAmount);
					Coin coin = new Coin(value);
					coin.velocity = new Vector2(Mathf.RandomFloat(-1, 1), 2) * 5;
