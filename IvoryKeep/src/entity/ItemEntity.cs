...
{
	const float ITEM_INFO_DELAY = 0.5f;
	...
	{
		return !(stuck && item.type == ItemType.Ammo);
	}
	...
	void onHit(bool x, bool y)
	{
		...
		if (velocity.lengthSquared > 4 * 4)
		{
			if (tile != null)
			{
				...
				//GameState.instance.level.addEntity(Effects.CreateImpactEffect(normal, velocity.length, Mathf.ARGBToVector(color).xyz), position + velocity.normalized * 0.01f);
			}
		}
		...
		if (item.projectileSticks && thrower != null && (tile == null || !tile.breaksArrows))
		{
			stuck = true;
			stuckTile = pos;
	...
	public override void update()
	{
		...
		{
			/*
			if (!GameState.instance.level.overlapTiles(position - 0.25f, position + 0.25f))
			{
				stuck = false;
				gravity = -20;
			}
		...
		else if (item.tumbles) //if (item.projectileItem && damage == 0 && thrower != null)
		{
			...
			else
			{
				if (MathF.Abs(rotation % (MathF.PI * 2)) < 0.5f * MathF.PI)
					rotation = Mathf.Lerp(rotation, 0, 5 * Time.deltaTime);
				else
					rotation = Mathf.LerpAngle(rotation, MathF.PI, 5 * Time.deltaTime);
			}
		}
			...
			if (hit != null)
			{
				if (hit.entity != null && hit.entity != this)
				{
					if (hit.entity is Hittable && !hitEntities.Contains(hit.entity) && velocity.lengthSquared > 4 * 4)
					{
						...
						if (player != null)
						{
							skipHit = ricochets < 0 || (hit.entity == thrower && ricochets == 0) || (Time.currentTime - throwTime) / 1e9f < 0.1f;
							Vector2 relativeVelocity = velocity - player.velocity;
							if (relativeVelocity.length < 8)
								skipHit = true;
						}

						if (!skipHit)
						{
							if (hittable.hit(thrower == null ? 0 : damage, this, item))
							{
	...
	public override void render()
	{
		...
		if (outline != 0 && velocity.lengthSquared < 1 && player.velocity.lengthSquared < 4.0f && player.actions.currentAction == null)
		{
			...
			if (player.numOverlaysOpen == 0 && Time.gameTime - timeInFocus > ITEM_INFO_DELAY)
			{
