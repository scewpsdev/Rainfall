...
public class LightningProjectile : Entity
{
		...
		for (int i = 0; i < numHits; i++)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						float deviation = Mathf.RandomFloat(-1, 1);
	...
	public override void render()
	{
		...
		for (int i = 0; i < cornerPoints.Count; i++)
		{
			...
			float startTime = Mathf.Lerp(start.z, end.z, 0.5f);
			...
			for (int j = 0; j < 3; j++)
			{
				...
				Vector4 color = Mathf.ARGBToVector(colorSelection == 0 ? 0xFF5b98ff : colorSelection == 1 ? 0xFF6edcff : 0xFFd6eeff) * new Vector4(1.5f);
			...
			for (int j = 0; j < (int)MathF.Ceiling(length); j++)
			{
				...
				float startTime = Mathf.Lerp(start.z, end.z, progress);
				float elapsed = Time.currentTime / 1e9f - startTime;
