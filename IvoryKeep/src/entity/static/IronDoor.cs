...
public class IronDoor : Entity, Interactable, Hittable
{
		...
		else
		{
			...
			for (int i = 0; i < numHits; i++)
			{
				...
				if (entity is Player || entity is Mob || entity is ItemEntity)
				{
					...
					if (Mathf.Fract(pos.x) < 0.5f && (level.getTile(tile.x - 1, tile.y) == null || !level.getTile(tile.x - 1, tile.y).isSolid))
	...
	public override void update()
	{
		...
		openProgress = Mathf.Lerp(openProgress, openDst, 10 * Time.deltaTime);
