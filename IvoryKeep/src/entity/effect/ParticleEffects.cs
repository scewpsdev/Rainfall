...
public static unsafe class ParticleEffects
{
	...
	public static ParticleEffect CreateBreakEffect(int count, Vector3 color)
	{
		ParticleEffect effect = new ParticleEffect(null, "effects/break.rfs");
		effect.systems[0].handle->bursts[0].count = count;
		effect.systems[0].handle->colorAnim.value0.value.xyz = color;
		effect.systems[0].handle->colorAnim.value1.value.xyz = color;
		effect.collision = true;
	...
	public static ParticleEffect CreateStepEffect(int count, float duration, Vector3 color, Entity entity)
	{
		ParticleEffect effect = new ParticleEffect(entity, "effects/impact.rfs");
		effect.systems[0].handle->startVelocity = new Vector3(0, 2, 0);
		effect.systems[0].handle->randomVelocity *= 0.3f;
		effect.systems[0].handle->gravity = -5;
		effect.systems[0].handle->bursts[0].count = count;
	...
	public static ParticleEffect CreateDestroyWoodEffect(uint color, int count = 10, Vector2 initialVelocity = default)
	{
		...
		effect.systems[0].handle->color = Mathf.ARGBToVector(color);
	...
	public static ParticleEffect CreateTeleportEffect(Entity entity, uint color)
	{
		...
		effect.systems[0].handle->colorAnim.value0.value.xyz = Mathf.ARGBToVector(color).xyz;
		effect.systems[0].handle->colorAnim.value1.value.xyz = Mathf.ARGBToVector(color).xyz;
		return effect;
	}
	...
	public static ParticleEffect CreateCriticalEffect()
	{
		...
		effect.systems[0].handle->color = 0xFFb70f0f;
		return effect;
	}
	...
	public static ParticleEffect CreateScrollUseEffect(Entity follow, uint color = 0xFFFFFFFF)
	{
		ParticleEffect effect = new ParticleEffect(follow, "effects/scroll_use.rfs");
		effect.systems[0].handle->colorAnim.value0.value.xyz = Mathf.ARGBToVector(color).xyz;
		effect.systems[0].handle->colorAnim.value1.value.xyz = Mathf.ARGBToVector(color).xyz;
		effect.systems[0].handle->colorAnim.value2.value.xyz = Mathf.ARGBToVector(color).xyz;
		return effect;
	}

	public static ParticleEffect CreateCastEffect()
	{
		ParticleEffect effect = new ParticleEffect(null, "effects/cast.rfs");
		return effect;
	}

	public static ParticleEffect CreateMagicTrailEffect(Entity follow)
	{
		ParticleEffect effect = new ParticleEffect(follow, "effects/magic_trail.rfs");
		effect.collision = true;
		return effect;
	}
