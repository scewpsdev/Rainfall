		...
		for (int j = 0; j < systems.Length; j++)
		{
			...
			for (int i = 0; i < systems[j].numParticles; i++)
			{
				...
				if (particle.active)
				{
					...
					Renderer.DrawSprite(particle.position.x - 0.5f * size, particle.position.y - 0.5f * size, layer, size, size, particle.rotation, textureAtlases[j], u0, v0, w, h, particle.color * (1 + systems[j].handle->emissiveIntensity), systems[j].handle->additive);
				}
			}
		}
	}
