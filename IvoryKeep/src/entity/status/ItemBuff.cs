...
public class ItemBuff
{
	...
	public void render(Entity entity)
	{
		if (renderAura)
		{
			...
			float alpha = 1 - MathF.Exp(-auraStrength * 1.0f);
			Renderer.DrawSprite(center.x - 0.5f * size, center.y - 0.5f * size, Entity.LAYER_BG, size, size, 0, aura, false, Mathf.ARGBToVector(auraColor) * new Vector4(alpha, alpha, alpha, 1), true);
		}
	}
}
