...
public class HUD2
{
	...
	void renderMessages()
	{
		...
		for (int i = 0; i < messages.Count; i++)
		{
			...
			float elapsed = (Time.currentTime - notif.timeSent) / 1e9f;
			float alpha = elapsed < MESSAGE_SHOW_DURATION - 1 ? 1 : Mathf.Lerp(1, 0, (elapsed - MESSAGE_SHOW_DURATION + 1) / 1);
			uint color = Mathf.ColorAlpha(0xFFAAAAAA, alpha);
	...
	void renderPopup()
	{
		...
		if (text != null && elapsed < POPUP_DURATION)
		{
			for (int i = 0; i < text.Length; i++)
			{
				...
				float yanim = Mathf.Lerp(0, -Renderer.UIHeight / 8, progress);
				yanim = 0;
				float alpha = elapsed < 1 ? elapsed : elapsed > POPUP_DURATION - 2 ? (1 - 0.5f * (elapsed - (POPUP_DURATION - 2))) : 1;
				uint color = Mathf.ColorAlpha(0xFFAAAAAA, alpha);
				Renderer.DrawUIText(Renderer.UIWidth / 2 - size.x / 2, Renderer.UIHeight / 4 + (-text.Length + i) * (size.y + 1) + (int)yanim + 1, text[i], 1, Mathf.ColorAlpha(0xFF000000, alpha));
	...
	void ___renderHandItems(bool flipItems)
	{
		...
		float alpha = player.position.y < GameState.instance.camera.bottom + 0.1f * GameState.instance.camera.height &&
			...
			? 0.2f : 1.0f;
		alpha = 1;
		uint frameColor = Mathf.ColorAlpha(0xFF555555, alpha);
		uint bgColor = Mathf.ColorAlpha(0xFF222222, alpha);
		uint txtColor = Mathf.ColorAlpha(0xFFBBBBBB, alpha);
	...
	void ___renderQuickItems(bool flipItems)
	{
		...
		uint frameColor = Mathf.ColorAlpha(0xFF555555, alpha);
		uint bgColor = Mathf.ColorAlpha(0xFF222222, alpha);
		uint selectionColor = Mathf.ColorAlpha(0xFF777777, alpha);
		uint txtColor = Mathf.ColorAlpha(0xFFBBBBBB, alpha);
		...
		if (player.activeItems[player.selectedActiveItem] != null)
		{
			...
			{
				float txtAlpha = elapsed < ITEM_NAME_DURATION - 1 ? 1 : Mathf.Lerp(1, 0, (elapsed - ITEM_NAME_DURATION + 1) / 1);
				uint color = Mathf.ColorAlpha(txtColor, txtAlpha);
	...
	public void render()
	{
		...
		if (screenFade != 1)
		{
			Renderer.DrawUISprite(0, 0, Renderer.UIWidth, Renderer.UIHeight, null, false, Mathf.ColorAlpha(0xFF000000, 1 - screenFade));
		}
	}
}
