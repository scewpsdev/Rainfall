...
public class Player : Entity, Hittable
{
	...
	public Player()
	{
		...
		//model = Resource.GetModel("cloth.gltf");
	...
	public override void init()
	{
		...
		hitboxFilterGroup = PhysicsFilter.PlayerHitbox;
		load("entity/player/player.rfs");

		controller = new CharacterController(this, 0.3f, Vector3.Zero, 2.0f, 0.1f, PhysicsFilter.Default | PhysicsFilter.Creature);
	...
	public void snapInputPosition()
	{
		...
		{
			Vector3 moveVelocity = Quaternion.AxisAngle(Vector3.Up, GameState.instance.camera.yaw) * moveDelta;
	...
	public void hit(int damage, bool criticalHit, Vector3 hitDirection, Entity by, Item item, RigidBody hitbox)
	{
		if (parryItem != null)
		{
			if (by is Creature)
			{
				Creature creature = by as Creature;
				creature.stagger();
			}
		}
	...
	public void stagger()
	{
		...
		actionManager.queueAction(new StaggerAction());
	}
	...
	public override void fixedUpdate(float delta)
	{
		...
		{
			float speed = 6;
			Vector3 moveVelocity = Quaternion.AxisAngle(Vector3.Up, GameState.instance.camera.yaw) * moveDelta.normalized * speed;
			...
			if (actionManager.currentAction == null || !actionManager.currentAction.rotationIsLocked)
				yaw = Mathf.LinearAngle(yaw, -moveVelocity.xz.angle - MathF.PI * 0.5f, 12 * delta);
			rotation = Quaternion.AxisAngle(Vector3.Up, yaw);
		}
		else
		{
		...
		displacement.xz += (Quaternion.AxisAngle(Vector3.Up, MathF.PI) * rootMotionDelta).xz;
		...
		if ((collisionFlags & ControllerCollisionFlag.Down) != 0)
			velocity.y = Mathf.Lerp(velocity.y, 0, 10 * delta);
		//position += displacement;

		PhysicsHit? hit = Physics.SweepSphere(controller.radius, position + Vector3.Up * controller.radius + 0.2f, Vector3.Down, 0.2f + controller.radius + 0.2f, QueryFilterFlags.Default);
		...
		for (int i = 0; i < armor.Length; i++)
		{
			if (armor[i] != null)
			{
				...
				armorEntity[i].rotation = rotation * Quaternion.AxisAngle(Vector3.Up, MathF.PI);
			}
		}
	...
	public override void update()
	{
		...
		{
			animator.getRootMotion(actionManager.currentActionAnim, out Vector3 rootMotionDisplacement, out Quaternion rootMotionRotation, out bool hasLooped);
			if (lastRootMotionAction == actionManager.currentAction && lastRootMotionAnim == actionManager.currentActionAnim && !hasLooped)
			{
				rootMotionDelta += rotation * (rootMotionDisplacement - lastRootMotionDisplacement) * actionManager.currentAction.rootMotionMultiplier;
			}
