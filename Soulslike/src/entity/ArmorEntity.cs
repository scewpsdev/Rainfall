...
public class ArmorEntity : Entity
{
	...
	public ArmorEntity(Armor armor, Player player)
	{
		...
		if (armor.cloth != null)
		{
			...
			cloth = new Cloth(armor.cloth, 0, animator, player.position, player.rotation * Quaternion.AxisAngle(Vector3.Up, MathF.PI), clothParams);
	...
	{
		Cloth.SetWind(new Vector3(1, 0, -3) * 0.5f * Mathf.Remap(simplex.sample1f(Time.gameTime), -1, 1, 0.9f, 1.2f));
