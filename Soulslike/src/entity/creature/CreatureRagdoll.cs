...
public class CreatureRagdoll : Entity
{
	...
	public CreatureRagdoll(Creature creature, string rootNode, Vector3 knockback, RigidBody hitbox)
	{
		...
		if (creature.hitboxes != null)
		{
			...
			{
				//Vector3 localVelocity = Quaternion.AxisAngle(Vector3.Up, creature.yaw).conjugated * creature.velocity;
				ragdoll = new Ragdoll(this, model.skeleton.getNode(rootNode), animator, getModelMatrix(), creature.velocity, creature.hitboxData, PhysicsFilter.Ragdoll, PhysicsFilter.Default);
	...
	public override void init()
	{
		...
		foreach (RigidBody body in ragdoll.hitboxes)
			body.scale(scale.max);
	}
