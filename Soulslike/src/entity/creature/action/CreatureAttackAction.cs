...
public struct CreatureAttack
{
	...
	public float triggerDistanceMin = 2;
...
public class CreatureAttackAction : CreatureAction
{
	...
	void processHit(ref PhysicsHit hit, Vector3 direction, Creature player)
	{
		...
		if (entity is Hittable)
		{
			...
			if (!hitEntities.Contains(entity))
			{
				...
				hittable.hit((int)MathF.Ceiling(damage), false, direction, player, null, hit.body);
		...
		{
			float bladeLength = player.weaponReach;
