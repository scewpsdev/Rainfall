...
public class LadderClimbAction : PlayerAction
{
	...
	public LadderClimbAction(int idx = -1, bool up = true)
		: base("ladder_climb", (idx + (up ? 0 : 1)) % 2)
	{
		...
		if (idx != -1)
			addSoundEffect(new ActionSfx(Resource.GetSounds("audio/climb", 5), 1, 1, followUpCancelTime, true));
	...
	public override void update(Player player)
	{
		...
		else
		{
			if (InputManager.IsDown("MoveForward"))
				player.actionManager.queueAction(new LadderClimbAction(idx + 1, true));
			else if (InputManager.IsDown("MoveBack"))
				player.actionManager.queueAction(new LadderClimbAction(idx + 1, false));
		}
