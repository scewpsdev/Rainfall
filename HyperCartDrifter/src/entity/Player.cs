...
public class Player : Entity
{
	...
	public override void destroy()
	{
		...
		Resource.FreeModel(cap);
		Resource.FreeModel(chain);
		Resource.FreeModel(glasses);
	...
	public override void update()
	{
		if (ragdoll != null)
		{
			...
			if ((Time.currentTime - ragdollSince) / 1e9f >= 3 || velocity.length < 0.5f || velocity.y < -1000)
			{
				cart.respawn();
				ragdoll.destroy();
				ragdoll = null;
		...
		else
		{
			...
			Vector3 velocity = cart.body.getVelocity();
			if (velocity.y < -1000)
				cart.respawn();
