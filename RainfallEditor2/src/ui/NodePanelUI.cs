...
public static partial class EditorUI
{
	...
	static unsafe void NodePanel(EditorInstance instance)
	{
		...
		ImGui.SetNextWindowSize(new Vector2(NODE_PANEL_WIDTH, (Display.height - ImGui.GetFrameHeight() * 2) / 2));
	...
	static unsafe void SceneSettingsList(EditorInstance instance)
	{
		DragFloat(instance, "Simulate entity velocity", "scene_simulate_entity_velocity", ref instance.simulatedEntityVelocity, 0.1f, -100, 100);
	}

	static unsafe void ScenePanel(EditorInstance instance)
	{
		ImGui.SetNextWindowPos(new Vector2(0, ImGui.GetFrameHeight() * 2 + (Display.height - ImGui.GetFrameHeight() * 2) / 2));
		ImGui.SetNextWindowSize(new Vector2(NODE_PANEL_WIDTH, (Display.height - ImGui.GetFrameHeight() * 2) / 2));
		if (ImGui.Begin("Scene", null, ImGuiWindowFlags.NoMove | ImGuiWindowFlags.NoResize | ImGuiWindowFlags.NoCollapse))
		{
			SceneSettingsList(instance);
		}
		ImGui.End();
	}
}
