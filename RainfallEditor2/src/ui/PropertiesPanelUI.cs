...
public static partial class EditorUI
{
		...
		if (ImGui.TreeNodeEx("Collider", ImGuiTreeNodeFlags.SpanAvailWidth | ImGuiTreeNodeFlags.DefaultOpen))
		{
			...
			for (int i = 0; i < entity.data.colliders.Count; i++)
			{
				...
				else if (collider.type == SceneFormat.ColliderType.Mesh || collider.type == SceneFormat.ColliderType.ConvexMesh)
				{
					...
					{
						collider.meshCollider = collider.meshColliderPath != null ? Resource.GetModel(RainfallEditor2.CompileAsset(collider.meshColliderPath)) : null;
						instance.notifyEdit();
					}
				}
			...
			if (entity.data.boneColliders != null)
			{
				if (entity.data.model != null)
				{
					...
					if (ImGui.Button("Copy Left to Right"))
					{
						for (int i = 0; i < entity.data.model.skeleton.nodes.Length; i++)
						{
							...
							if (deforming && node.name.EndsWith(".L", StringComparison.InvariantCultureIgnoreCase))
							{
								if (entity.data.boneColliders.ContainsKey(node.name))
								{
									string otherBone = node.name.Substring(0, node.name.Length - 2) + ".R";
									if (entity.data.boneColliders.ContainsKey(otherBone))
									{
										SceneFormat.ColliderData collider = entity.data.boneColliders[node.name];
										SceneFormat.ColliderData otherCollider = entity.data.boneColliders[otherBone];

										otherCollider = collider;
										otherCollider.offset = otherCollider.offset * new Vector3(-1, 1, 1);
										otherCollider.eulers = otherCollider.eulers * new Vector3(1, -1, -1);
										entity.data.boneColliders[otherBone] = otherCollider;
									}
								}
							}
						}
	...
	static unsafe void Lights(Entity entity, EditorInstance instance)
	{
		if (ImGui.TreeNodeEx("Lights", ImGuiTreeNodeFlags.SpanAvailWidth | ImGuiTreeNodeFlags.DefaultOpen))
		{
			...
			for (int i = 0; i < entity.data.lights.Count; i++)
			{
				...
				Vector2i screenPosition = Mathf.WorldToScreenSpace(lightGlobalPosition, pv, Display.viewportSize);
				int width = 64;
				int height = 64;
	...
	static unsafe void PropertiesPanel(EditorInstance instance, RainfallEditor2 editor)
	{
