			...
			for (int i = 0; entity.particles != null && i < entity.particles.Length; i++)
			{
				...
				if (particlesOpen)
				{
					...
					*/
					...
					if (particles->spawnShape == ParticleSpawnShape.Circle || particles->spawnShape == ParticleSpawnShape.Sphere || particles->spawnShape == ParticleSpawnShape.Box|| particles->spawnShape == ParticleSpawnShape.Line)
					{
					...
					if (particles->spawnShape == ParticleSpawnShape.Box)
					{
						DragFloat3(instance, "Spawn Size", "particle_spawn_size" + i, ref particles->spawnSize, 0.005f);
					}
					if (particles->spawnShape == ParticleSpawnShape.Line)
					{
						DragFloat3(instance, "Spawn Point 1", "particle_spawn_point0" + i, ref particles->spawnPoint0, 0.005f);

						DragFloat3(instance, "Spawn Point 2", "particle_spawn_point1" + i, ref particles->spawnPoint1, 0.005f);
					}
					...
					*/
					...
					DragFloat(instance, "Drag", "particle_drag" + i, ref particles->drag, 0.001f, 0, 1);
					...
					DragFloat(instance, "Random Velocity", "particle_random_velocity" + i, ref particles->randomVelocity, 0.001f, 0, 1);

					DragFloat(instance, "Random Direction", "particle_random_direction" + i, ref particles->randomDirection, 0.001f, 0, 1);

					bool randomDirectionUniform = particles->randomDirectionUniform;
					Checkbox(instance, "Random Direction Uniform", "particle_random_direction_uniform" + i, ref randomDirectionUniform);
					particles->randomDirectionUniform= randomDirectionUniform;
					...
					if (FileSelect("Texture Atlas", "particle_atlas" + i, ref textureAtlasPath, "png"))
					{
						...
						{
							entity.particles[i].textureAtlas = Resource.GetTexture(RainfallEditor2.CompileAsset(textureAtlasPath));
						}
						else
						{
							entity.particles[i].textureAtlas = null;
