...
public class MapLoader
{
	...
	static void LoadModel(string path, WorldManager world, Scene scene)
	{
		...
		for (int i = 0; i < model.skeleton.nodes.Length; i++)
		{
			...
			if (node.meshes != null && node.meshes.Length > 0)
			{
				...
				else if (node.getProperty("foliage", model))
				{
					Entity entity = new Foliage();
					entity.model = model;
	...
	static void LoadNodes(string path, WorldManager world, Scene scene)
	{
		...
		for (int i = 0; i < model.skeleton.nodes.Length; i++)
		{
			...
			if (node.name.StartsWith("__ cubemap"))
			{
				string name = null;
				if (node.name.StartsWith("__ cubemap "))
				{
					name = node.name.Substring(11);
					int spaceIdx = name.LastIndexOf(' ');
					if (spaceIdx != -1)
						name = name.Substring(0, spaceIdx);
				}
			...
			else if (node.name.StartsWith("__ spawn "))
			{
				string spawnName = node.name.Substring(9);
				world.spawnPoints.Add(spawnName, node.transform);
			}
			...
			else if (node.name.StartsWith("__ creature "))
			{
				string objectType = node.name;
				if (objectType.LastIndexOf(' ') > 11)
					objectType = objectType.Substring(0, objectType.LastIndexOf(' '));
				objectType = objectType.Substring(12);
