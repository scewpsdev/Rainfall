...
public class FirstPersonActionManager
{
	...
	public List<FirstPersonAction> actionQueue = new List<FirstPersonAction>();
	public FirstPersonAction currentAction { get => actionQueue.Count > 0 ? actionQueue[0] : null; }
	public int size { get => actionQueue.Count; }


	public FirstPersonActionManager(Player player)
	{
		this.player = player;
	}

	void initializeAction(FirstPersonAction currentAction)
	{
		// Initialize action
		...
		for (int i = 0; i < 3; i++)
		{
			...
			if (currentAction.animationName[i] != null && currentAction.animationSet[i] != null)
			{
				...
				anim.layers[(i + 1) % 3] = new AnimationLayer(currentAction.animationSet[i], currentAction.animationName[i], false);
			...
			if (currentAction.animationName[i] != null && animationData == null)
			{
				...
				anim.layers[(i + 1) % 3] = new AnimationLayer(player.model, currentAction.animationName[i], false);
	...
	void deinitializeAction(FirstPersonAction currentAction)
	{
	...
	public void update()
	{
		...
		{
			FirstPersonAction currentAction = actionQueue[0];
	...
	public void queueAction(FirstPersonAction action)
	{
		bool enoughStamina = true; //action.staminaCost == 0.0f || player.stats.stamina > 0;
		bool enoughMana = true; //action.manaCost == 0 || player.stats.mana >= action.manaCost;
	...
	public void setAction(FirstPersonAction action)
	{
		cancelAllActions();
		queueAction(action);
	}
