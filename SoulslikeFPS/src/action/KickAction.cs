...
public class KickAction : FirstPersonAction
{
	bool hasKicked;
	...
	public override void update(Player player)
	{
		...
		if (elapsedTime > 12 / 24.0f && !hasKicked)
		{
			...
			int numHits = Physics.OverlapCapsule(0.25f, range, origin + direction * 0.5f * range, player.rotation * Quaternion.AxisAngle(Vector3.Right, MathF.PI * 0.5f), hits, QueryFilterFlags.Default, PhysicsFilter.Default | PhysicsFilter.CreatureHitbox);
			for (int i = 0; i < numHits; i++)
			{
				...
				if (body.type == RigidBodyType.Dynamic)
				{
					...
					body.setAngularVelocity(Mathf.RandomVector3(-1, 1));
				}
