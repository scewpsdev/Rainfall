...
public class FirstPersonController
{
	...
	{
		this.player = player;
		controller = new CharacterController(player, COLLIDER_RADIUS, Vector3.Zero, COLLIDER_HEIGHT, 0.2f, PhysicsFiltering.DEFAULT | PhysicsFiltering.CREATURE, new CollisionCallback(this));
	}
...

		{
			//if (currentAction == null || !currentAction.lockMovement)
			{
				...
				if (InputManager.IsDown("MoveRight"))
					fsu.x++;
				if (InputManager.IsDown("MoveBack"))
					fsu.z--;
		...
		//if (currentAction == null || currentAction.movementSpeedMultiplier > 0.0f)
		{
			...
			if (InputManager.IsPressed("Crouch"))
			{
				...
				else
				{
					...
					{
						Span<HitData> hits = stackalloc HitData[16];
						//int numHits = Physics.SweepSphere(COLLIDER_RADIUS, player.position + new Vector3(0.0f, PLAYER_HEIGHT_DUCKED - PLAYER_RADIUS, 0.0f), Vector3.Up, PLAYER_HEIGHT_STANDING - PLAYER_HEIGHT_DUCKED, hits, QueryFilterFlags.Static);
						int numHits = 0;
