...
public class Player : Entity, Hittable
{
	...
	public FirstPersonActionManager actionManager;
	...
	public Weapon parryItem = null;
	public int parryItemHand;
	public Weapon blockItem = null;
	public int blockItemHand;
	...
	public unsafe Player(Camera camera)
	{
		...
		rightShoulderNode = model.skeleton.getNode("clavicle_r");
		leftShoulderNode = model.skeleton.getNode("clavicle_l");
		...
		idleAnim.animationSpeed = 0.003f;
		runAnim = Animator.CreateAnimation(model, [new AnimationLayer(model, "run", true), null, null], 0.4f);
		fallAnim = Animator.CreateAnimation(model, [new AnimationLayer(model, "fall", true), null, null], 0.7f);
		...
		actionManager = new FirstPersonActionManager(this);
	...
	public override void init()
	{
		...
		controller.onStep = () =>
		{
			...
			Audio.PlayOrganic(stepSound, position + rotation * Vector3.Forward, 0.3f);
	...
	{
		this.yaw = yaw;
		rotation = Quaternion.AxisAngle(Vector3.Up, yaw);
	}
	...
	{
		if (parryItem != null && hit.blockable)
		{
			actionManager.setAction(new ParryHitAction(parryItem, parryItemHand));

			hit.damage = 0;

			if (hit.by is Creature)
			{
				Creature creature = hit.by as Creature;
				creature.stagger();
			}
		}
		else if (blockItem != null && hit.blockable)
		{
			actionManager.setAction(new BlockHitAction(blockItem, blockItemHand));

			hit.damage = 0;
		}
		else if (!isInvincible && hit.stagger > 0)
		{
			stagger(hit.stagger);
		}
	...
	public void stagger(int level)
	{
		actionManager.cancelAllActions();
		actionManager.queueAction(new StaggerAction(level));
	}
	...
	void updateMovement()
	{
		...
		pitch = Mathf.Clamp(pitch - Input.cursorMove.y * 0.001f, -MathF.PI * 0.5f, MathF.PI * 0.5f);
		if ((actionManager.currentAction == null || !actionManager.currentAction.lockYaw) && controller.currentLadder == null)
		{
			...
			if (camerayaw != 0)
			{
				float newCameraYaw = Mathf.Lerp(camerayaw, 0, 10 * Time.deltaTime);
		...
		{
			camerayaw = Mathf.Clamp(camerayaw - Input.cursorMove.x * 0.001f, -MathF.PI * 0.75f, MathF.PI * 0.75f);
		}
		...
		camera.setTransform(getModelMatrix() * Matrix.CreateTranslation(0, cameraHeight, 0) * Matrix.CreateRotation(Vector3.Up, camerayaw) * Matrix.CreateRotation(Vector3.Right, pitch) * animator.getNodeTransform(cameraNode));
	...
	Matrix calculateWeaponSway(int side)
	{
		...
		viewmodelVerticalSpeedAnim = Mathf.Lerp(viewmodelVerticalSpeedAnim, verticalSpeedAnimDst * 0.0075f, 5 * Time.deltaTime);
	...
	void updateAnimations()
	{
		...
		Matrix rightSway = calculateWeaponSway(0);
		rightHandAnimator.setNodeLocalTransform(rightShoulderNode, rightSway * rightHandAnimator.getNodeLocalTransform(rightShoulderNode));
		rightHandAnimator.setNodeLocalTransform(rightWeaponNode, rightSway * rightHandAnimator.getNodeLocalTransform(rightWeaponNode));

		Matrix leftSway = calculateWeaponSway(rightWeapon == null ? 1 : 0);
		leftHandAnimator.setNodeLocalTransform(leftShoulderNode, leftSway * leftHandAnimator.getNodeLocalTransform(leftShoulderNode));
		leftHandAnimator.setNodeLocalTransform(leftWeaponNode, leftSway * leftHandAnimator.getNodeLocalTransform(leftWeaponNode));
		...
		currentViewmodelAim = Mathf.Lerp(currentViewmodelAim, viewmodelAim, 5 * Time.deltaTime);
