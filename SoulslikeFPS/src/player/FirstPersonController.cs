	...
	Vector3 updateMovementInputs()
	{
		...
		*/
		...
		if (isCursorLocked && (currentAction == null || !currentAction.lockRotation))
		{
			...
			yaw -= lookVector.x;
			pitch = Math.Clamp(pitch - lookVector.y, -Mathf.PiOver2, Mathf.PiOver2);
		}
		*/
		...
		if (moveType == MoveType.Walk)
		{
			...
			// Position update
			{
				...
				if (isGrounded && currentAction != null && currentAction.rootMotion)
				{
					...
					rootMotionDisplacement = Quaternion.AxisAngle(Vector3.Up, MathF.PI + yaw) * rootMotionDisplacement;
