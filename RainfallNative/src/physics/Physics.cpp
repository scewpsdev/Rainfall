...
RFAPI void Physics_Update(float delta)
{
	...
	int64_t beforeUpdate = Platform_GetTimestamp();
	...
	int64_t afterUpdate = Platform_GetTimestamp();
	if (numSteps > 0)
...
RFAPI void Physics_RigidBodySetActive(RigidBody* body, bool active)
{
	if (!body->actor->getScene() && active)
		scene->addActor(*body->actor);
	else if (body->actor->getScene() && !active)
		scene->removeActor(*body->actor);
}
...
RFAPI void Physics_RigidBodyScale(RigidBody* body, float scale)
{
	PxScaleRigidActor(*body->actor, scale);
}
...
RFAPI void Physics_RigidBodyGetWorldAABB(RigidBody* body, Vector3* outPosition, Vector3* outSize)
{
	PxBounds3 bounds = body->actor->getWorldBounds();
	PxVec3 center = bounds.getCenter();
	PxVec3 size = bounds.getDimensions();
	*outPosition = Vector3(center.x, center.y, center.z);
	*outSize = Vector3(size.x, size.y, size.z);
}
