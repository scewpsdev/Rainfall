...
static Quaternion ExtractRotation(Matrix matrix, const Vector3& scale)
{
	float x, y, z, w;

	float trace = matrix.m00 + matrix.m11 + matrix.m22; // I removed + 1.0f; see discussion with Ethan
...
void Matrix::decompose(Vector3& translation, Quaternion& rotation, Vector3& scale) const
{
...
Vector4& Matrix::operator[](int column)
{
	return columns[column];
}

const Vector4& Matrix::operator[](int column) const
{
	return columns[column];
}

Matrix Matrix::Translate(const Vector4& v)
{
...
Matrix Matrix::Transform(const Vector3& position, const Quaternion& rotation, const Vector3& scale)
{
