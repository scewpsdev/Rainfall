...
static void ReadNode(bx::ReaderI* reader, NodeData& node, bx::Error* err)
{
	...
	bx::read(reader, node.numProperties, err);
	if (node.numProperties)
	{
		node.properties = (CustomProperty*)BX_ALLOC(Application_GetAllocator(), sizeof(CustomProperty) * node.numProperties);
		memset(node.properties, 0, sizeof(CustomProperty) * node.numProperties);
		for (int i = 0; i < node.numProperties; i++)
		{
			bx::read(reader, node.properties[i].name, err);
			bx::read(reader, node.properties[i].type, err);
			if (node.properties[i].type <= CustomPropertyType::Double)
			{
				bx::read(reader, *(uint64_t*)&node.properties[i].data, err);
			}
			else if (node.properties[i].type == CustomPropertyType::String)
			{
				int strLength;
				bx::read(reader, strLength, err);
				char* str = (char*)BX_ALLOC(Application_GetAllocator(), strLength + 1);
				bx::read(reader, str, (strLength * 4 + 3) / 4, err);
				str[strLength] = 0;
				node.properties[i].data = str;
			}
			else if (node.properties[i].type == CustomPropertyType::Vector3)
			{
				node.properties[i].data = BX_ALLOC(Application_GetAllocator(), sizeof(Vector3));
				bx::read(reader, node.properties[i].data, sizeof(Vector3), err);
			}
		}
	}
