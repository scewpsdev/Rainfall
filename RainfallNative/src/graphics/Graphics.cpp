...
RFAPI Texture Graphics_CreateTextureImmutable(int width, int height, bgfx::TextureFormat::Enum format, bool hasMips, int numLayers, uint64_t flags, const bgfx::Memory* memory, bgfx::TextureInfo* info)
{
	bgfx::TextureHandle handle = bgfx::createTexture2D(width, height, hasMips, numLayers, format, flags, memory);
	bgfx::calcTextureSize(*info, width, height, 0, false, hasMips, numLayers, format);
	return handle.idx;
}
	...
	for (int i = 0; i < numAttachments; i++)
	{
		...
		if (texture.idx == bgfx::kInvalidHandle)
		{
			...
			if (attachmentInfo[i].ratio < bgfx::BackbufferRatio::Count)
				texture = bgfx::createTexture2D(attachmentInfo[i].ratio, attachmentInfo[i].generateMipmaps, 1, attachmentInfo[i].format, attachmentInfo[i].flags);
			else
				texture = bgfx::createTexture2D(attachmentInfo[i].width, attachmentInfo[i].height, attachmentInfo[i].generateMipmaps, 1, attachmentInfo[i].format, attachmentInfo[i].flags);

			bgfx::calcTextureSize(textureInfos[i], (uint16_t)attachmentInfo[i].width, (uint16_t)attachmentInfo[i].height, 1, false, attachmentInfo[i].generateMipmaps, 1, attachmentInfo[i].format);
		...
		attachments[i].init(texture, attachmentInfo[i].generateMipmaps && attachmentInfo[i].format == bgfx::TextureFormat::D24 ? bgfx::Access::ReadWrite : bgfx::Access::Write, BGFX_CUBE_MAP_POSITIVE_X + attachmentInfo[i].textureLayer, 1, 0, attachmentInfo[i].generateMipmaps ? BGFX_RESOLVE_AUTO_GEN_MIPS : BGFX_RESOLVE_NONE);
	}
...
RFAPI void Graphics_SetWriteMask(bool r, bool g, bool b, bool a, bool z)
{
	state &= 0xffffffffffffffff ^ BGFX_STATE_WRITE_MASK;
...
RFAPI uint32_t Graphics_ReadTexture(int pass, uint16_t texture, void* data)
{
	return bgfx::readTexture(bgfx::TextureHandle{ texture }, data);
}

RFAPI uint32_t Graphics_GetFrameIndex()
{
	return frameIdx;
}
