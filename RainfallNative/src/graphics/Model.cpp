...
RFAPI SceneData* Model_CreateFromParts(int numParts, int* _numVertices, PositionNormalTangent* _vertices, Vector2* _uvs, int* _numIndices, int* _indices)
{
	SceneData* sceneData = BX_NEW(Application_GetAllocator(), SceneData)();

	sceneData->numMeshes = numParts;
	sceneData->meshes = (MeshData*)BX_ALLOC(Application_GetAllocator(), numParts * sizeof(MeshData));
	...
	{
		int numVertices = _numVertices[i];
		int numIndices = _numIndices[i];

		PositionNormalTangent* vertices = CopyData(&_vertices[vertexOffset], numVertices);
		Vector2* uvs = CopyData(&_uvs[vertexOffset], numVertices);
		int* indices = CopyData(&_indices[indexOffset], numIndices);
...
RFAPI void Model_CreateMaterials(SceneData* scene, int numMaterials)
{
	scene->numMaterials = numMaterials;
	scene->materials = (MaterialData*)BX_ALLOC(Application_GetAllocator(), numMaterials * sizeof(MaterialData));
...
RFAPI void Model_ApplyMaterials(SceneData* scene)
{
