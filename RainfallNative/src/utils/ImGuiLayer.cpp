...
void ImGuiLayerInit()
{
	...
	io.KeyMap[ImGuiKey_LeftCtrl] = (int)KeyCode::Ctrl;
	io.KeyMap[ImGuiKey_LeftShift] = (int)KeyCode::Shift;
	io.KeyMap[ImGuiKey_LeftAlt] = (int)KeyCode::Alt;
	io.KeyMap[ImGuiKey_LeftSuper] = (int)KeyCode::Meta;
...
void ImGuiLayerBeginFrame()
{
	...
	int64_t now = Platform_GetTimestamp();
...
bool ImGuiLayerProcessEvent(const Event* ev)
{
	...
	case EventType::Key:
	{
		...
		if (imguiHovered || !keyEvent->down)
		{
			...
			io.KeyCtrl = io.KeysDown[(int)KeyCode::Ctrl] || io.KeysDown[(int)KeyCode::RightCtrl];
			io.KeyShift = io.KeysDown[(int)KeyCode::Shift] || io.KeysDown[(int)KeyCode::RightShift];
			io.KeyAlt = io.KeysDown[(int)KeyCode::Alt] || io.KeysDown[(int)KeyCode::RightAlt];
			io.KeySuper = io.KeysDown[(int)KeyCode::Meta] || io.KeysDown[(int)KeyCode::RightMeta];
