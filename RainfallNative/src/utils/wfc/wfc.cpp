...
RFAPI bool WFC_Run(OverlappingWFCOptions options, uint32_t seed, int* input, int inputWidth, int inputHeight, int* output)
{
	Array2D<int> inputArr(inputWidth, inputHeight);
	memcpy(inputArr.data.data(), input, inputWidth * inputHeight * sizeof(int));

	OverlappingWFC<int> wfc(inputArr, options, (int)seed);

	auto resultArr = wfc.run();
	if (resultArr.has_value())
	{
		memcpy(output, resultArr.value().data.data(), options.out_width * options.out_height * sizeof(int));
		return true;
	}
	return false;
}