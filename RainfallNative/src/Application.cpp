...
{
	double beforeFrame = glfwGetTime();
	//int64_t beforeFrame = Platform_GetTimestamp();
	if (!isPaused)
		currentFrame += beforeFrame - lastFrame;

	double maxDelta = 1.0 / 10;
	delta = (float)fmax(fmin(beforeFrame - lastFrame, maxDelta), 0.0);
	if (fpsCap != 0)
		delta = (float)fmax(delta, 1.0f / fpsCap);

	bool exit = !ProcessEvents(callbacks);

	if (beforeFrame - lastSecond > 1.0)
	{
	...
	if (fpsCap)
	{
		double frameLength = afterFrame - beforeFrame;
		double remaining = 1.0 / fpsCap - frameLength;
		if (remaining > 0)
		{
			Platform_SleepFor((int)(remaining * 1000));
		}
	}

	msCounter += (float)(afterFrame - beforeFrame) * 1000;
	frameCounter++;
...
{
	return (int64_t)(currentFrame * 1e9);
}
