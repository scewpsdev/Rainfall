...
vec3 RenderPointLight(vec3 position, vec3 normal, vec3 view, vec3 albedo, float roughness, float metallic, float ao, vec3 lightPosition, vec3 lightColor)
{
	...
	vec3 s = (specular + fLambert * kd * ao) * radiance * ndotwi * shadow;
...
// Environment mapping
vec3 RenderEnvironmentMap(vec3 normal, vec3 view, vec3 albedo, float roughness, float metallic, float ao, samplerCube environmentMap)
{
	vec3 f0 = mix(vec3_splat(0.04), albedo, metallic);
	...
	vec3 irradiance = SRGBToLinear(textureCubeLod(environmentMap, normal, 7.5).rgb);
	vec3 diffuse = irradiance * albedo;

	vec3 r = reflect(-view, normal);
	float lodFactor = (1.0 - exp(-roughness * 2.5)) * 12.0;
	vec3 prefiltered = SRGBToLinear(textureCubeLod(environmentMap, r, lodFactor).rgb);
