...
void main()
{
	...
	vec2 uv = (bladePosition.xz + offset) / terrainSize;
	float heightmapValue = texture2DLod(s_heightmap, uv, 0).r;
	...
	bladePosition.y = heightmapValue;

	bladePosition.y += heightmapValue;
	position.y += heightmapValue;
	worldPosition.y += heightmapValue;
