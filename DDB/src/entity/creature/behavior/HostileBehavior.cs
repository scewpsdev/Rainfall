...
internal class HostileBehavior : AIBehavior
{
	...
	OverlapHit[] hitBuffer = new OverlapHit[16];
	...
	void seekTarget()
	{
		int numHits = Physics.OverlapSphere(detectionRange, creature.position, hitBuffer, 16, QueryFilterFlags.Dynamic);
		for (int i = 0; i < numHits; i++)
		{
			RigidBody body = RigidBody.GetBodyFromHandle(hitBuffer[i].userData);
			if (body != null)
			{
