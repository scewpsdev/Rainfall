...
internal class HubArea : Entity
{
	...
	public HubArea(GraphicsDevice graphics)
	{
		...
		for (int x = -2; x < 2; x++)
		{
			for (int z = -2; z < 2; z++)
			{
				grassPatches.Add(new GrassPatch(terrain, new Vector2(x * GrassPatch.TILE_SIZE, z * GrassPatch.TILE_SIZE)));
			}
		}
	...
	public override void init()
	{
		...
		//level.addEntity(player = new Player(camera), new Vector3(-25, terrain.getHeight(-25, 11), 11), Quaternion.Identity);
		...
		for (int i = 0; i < 16; i++)
		{
			...
			level.addEntity(new Tree(), position, Quaternion.FromAxisAngle(Vector3.Up, MathHelper.RandomFloat(0.0f, MathF.PI * 2.0f, random)));
		}

		//level.addEntity(new SkeletonEnemy(), new Vector3(0.0f, 3.0f, 0.0f), Quaternion.Identity);

		level.addEntity(new Chest(new Item[] { Item.Get("quemick"), Item.Get("firebomb") }, new int[] { 2, 5 }), new Vector3(-2.5f, terrain.getHeight(-2.0f, 0.0f), 0.0f), Quaternion.Identity);
