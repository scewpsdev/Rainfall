...
internal class ShadowMap
{
	...
	static readonly float[] NEAR_PLANES = new float[] { -20.0f, -10.0f, 10.0f };
	public static readonly float[] FAR_PLANES = new float[] { 10.0f, 50.0f, 150.0f };
	...
	void calculateViewTransform(Vector3 cameraPosition, Quaternion cameraRotation, float cameraFov, float cameraAspect, float near, float far, out Matrix lightProjection, out Matrix lightView)
	{
		...
		lightProjection = Matrix.CreateOrthographic(size.x, size.y, -0.5f * size.z, 0.5f * size.z);
