...
public class Game3D<T> : Game where T : Game
{
	...
	protected int resolutionScale = 1;
	...
	public int compileResources(string projectPath, string outPath, string resCompilerPath)
	{
		return CompileFolder(projectPath, outPath + "\\assets", resCompilerPath);
	...
	public override void init()
	{
		...
		Renderer.Init(Display.width / resolutionScale, Display.height / resolutionScale, graphics);
	...
	{
		Renderer.Resize(width / resolutionScale, height / resolutionScale);
	}
	...
	static int CompileFolder(string folder, string outDir, string resCompilerPath)
	{
		...
		startInfo.Arguments = $"/C {resCompilerPath} res {outDir} png hdr ogg vsh fsh csh ttf rfs gltf glb";
