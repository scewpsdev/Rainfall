...
public class Scene
{
	...
	public Entity getEntityByName(string name, Entity parent = null)
	{
		if (parent != null)
		{
			for (int i = 0; i < parent.numChildren; i++)
			{
				...
				if (entity.name == name)
					return entity;
				Entity child = getEntityByName(name, entity);
				if (child != null)
					return child;
			}
		}
	...
	*/
	...
	public void fixedUpdate(float delta)
	{
		for (int i = 0; i < entities.Count; i++)
		{
			List<Entity> entityList = entities.ElementAt(i).Value;
			for (int j = 0; j < entityList.Count; j++)
			{
				entityList[j].fixedUpdate(delta);
			}
		}
	}
