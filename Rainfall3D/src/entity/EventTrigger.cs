...
public class EventTrigger : Entity
{
	Vector3 size, offset;
	uint filterMask;
	Action<EventTrigger, RigidBody, ContactType> callback;

	public EventTrigger(Vector3 size, Vector3 offset, Action<EventTrigger, RigidBody, ContactType> callback, uint filterMask = 1)
	{
		this.size = size;
		this.offset = offset;
	...
	{
		if (other != null)
			callback(this, other, contactType);
	}
}
