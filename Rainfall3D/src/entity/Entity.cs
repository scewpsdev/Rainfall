...
public class Entity : PhysicsEntity
{
	...
	public void addChild(Entity child, Matrix transform)
	{
		transform.decompose(out child.position, out child.rotation, out child.scale);
		addChild(child);
	}
	...
	{
		if (children != null && children.Count > idx)
			return children[idx];
		return null;
	}
	...
	public virtual void init()
	{
		if (body != null)
		{
			...
			transform.decompose(out Vector3 position, out Quaternion rotation);
	...
	{
		return getModelMatrix().translation;
	}
	...
	{
		return getModelMatrix().rotation;
	}
	...
	public Matrix localMatrix => Matrix.CreateTranslation(position) * Matrix.CreateRotation(rotation) * Matrix.CreateScale(scale);

	public Matrix getModelMatrix(Vector3 offset)
	{
		Matrix modelMatrix = Matrix.CreateTranslation(position + offset) * Matrix.CreateRotation(rotation) * Matrix.CreateScale(scale);
		if (parent != null)
