...
public class Enemy : Entity, Toucheable
{
	...
	public Enemy(Vector2 position)
	{
		...
		speed = TopDownTest.instance.manager.enemySpeed;
		health = TopDownTest.instance.manager.enemyHealth;
	}
	...
	void onDeath(Entity from)
	{
		...
		if (from != null)
		{
			int numXP = (TopDownTest.instance.player.luck + MathHelper.RandomInt(-TopDownTest.instance.player.luck / 2, TopDownTest.instance.player.luck / 2)) * 10;
			while (numXP > 0)
			{
			...
			float dropChance = 0.0001f * TopDownTest.instance.player.luck;
		...
		TopDownTest.instance.manager.enemiesKilled++;
	...
	void updateMovement()
	{
		...
		if (TopDownTest.instance.level.getTile(tilePos.x, tilePos.y) != 0)
			hit(10000, null);
