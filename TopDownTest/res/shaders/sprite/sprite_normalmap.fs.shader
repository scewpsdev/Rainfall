...
void main()
{
	...
	vec3 normalMapValue = normalize(2.0 * (SampleTextureByID(int(textureID + 0.5), v_texcoord0.xy + vec2(0.0, 0.5))).rgb - 1.0);
	
	vec3 normal = normalize(v_normal);
	vec3 tangent = cross(normalize(vec3(0.0, 1.0, -1.0)), normal);
	...
	vec3 normal2 = mul(tbn, normalMapValue);
    ...
    gl_FragData[1] = vec4(normal2 * 0.5 + 0.5, 1);
