...
public abstract class Mob : Entity, Hittable, StatusEffectReceiver
{
	...
	public override void render()
	{
		if (isVisible)
		{
			...
			if (sprite != null)
			{
				Renderer.DrawSprite(position.x - 0.5f, position.y - 0.5f, 0.001f, 1, 1, 0, shadow, false, 0x7FFFFFFF);

				if (hitMarker)
					Renderer.DrawVerticalSpriteSolid(position.x + rect.position.x, position.y, rect.position.y, rect.size.x, rect.size.y, sprite, direction == -1, 0xFFFFFFFF);
				else
					Renderer.DrawVerticalSprite(position.x + rect.position.x, position.y, rect.position.y, rect.size.x, rect.size.y, sprite, direction == -1, Vector4.One);

				if (outline != 0)
					Renderer.DrawVerticalSpriteSolid(position.x + rect.position.x, position.y + 0.001f, rect.position.y, rect.size.x, rect.size.y, sprite, direction == -1, 0xFFFFFFFF);
