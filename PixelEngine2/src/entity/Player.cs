		...
		if (isAlive && numOverlaysOpen == 0)
		{
			...
			else if (Settings.game.aimMode == AimMode.Directional)
			{
				if (Input.cursorHasMoved)
				{
					...
					if (MathF.Abs(Input.cursorPosition.x - playerScreenPos.x) > maxCursorDistance * 16 * PixelEngine2.instance.scale ||
						MathF.Abs(Input.cursorPosition.y - playerScreenPos.y) > maxCursorDistance * 16 * PixelEngine2.instance.scale)
					{
						int x = Math.Clamp(Input.cursorPosition.x, (int)MathF.Round(playerScreenPos.x - maxCursorDistance * 16 * PixelEngine2.instance.scale), (int)MathF.Round(playerScreenPos.x + maxCursorDistance * 16 * PixelEngine2.instance.scale));
						int y = Math.Clamp(Input.cursorPosition.y, (int)MathF.Round(playerScreenPos.y - maxCursorDistance * 16 * PixelEngine2.instance.scale), (int)MathF.Round(playerScreenPos.y + maxCursorDistance * 16 * PixelEngine2.instance.scale));
		...
		if (item.sprite != null)
		{
			...
			else
			{
				if (item != DefaultWeapon.instance)
				{
					...
					Renderer.DrawSprite(weaponPosition.x - 0.5f * item.size.x, weaponPosition.y - 0.5f * item.size.y, getWeaponOrigin(mainHand).z, item.size.x, item.size.y, 0, item.sprite, direction == -1, color);
					if (particles != null)
					{
