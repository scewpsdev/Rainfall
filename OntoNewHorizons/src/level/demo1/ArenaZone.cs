...
internal class ArenaZone : Entity
{
	...
	public override void init()
	{
		...
		OntoNewHorizons.instance.world.addEntity(ironBars = new IronBars(), new Vector3(-0.026f, -0.01f, 19.5f), Quaternion.Identity);
	}
	...
	void initRound()
	{
		...
		for (int i = 0; i < numEnemies; i++)
		{
			...
			OntoNewHorizons.instance.world.addEntity(enemy, new Vector3(x, y, z), Quaternion.FromAxisAngle(Vector3.Up, Random.Shared.NextSingle() * MathF.PI * 2.0f));
			enemies.Add(enemy);
		}
	}
	...
	void onLastKill(int round, Creature creature)
	{
		if (round == 6)
		{
			OntoNewHorizons.instance.world.addEntity(new ItemPickup(Item.Get("longbow")), creature.position + new Vector3(0.0f, 1.0f, 0.0f), Quaternion.Identity);
			ironBars.activate();
		}
	}
