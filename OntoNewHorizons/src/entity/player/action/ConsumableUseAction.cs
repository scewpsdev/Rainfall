...
internal class ConsumableUseAction : Action
{
	...
	public override void update(Player player)
	{
		...
		if (elapsedTime >= item.consumableUseTime && !wasUsed)
		{
			...
			{
				OntoNewHorizons.instance.world.addEntity(new Throwable(item, player, player.lookDirection, player.getWeaponTransform(handID).translation - player.lookOrigin), player.lookOrigin, player.lookRotation);
