...
internal class Throwable : Entity
{
	...
	public override void onContact(RigidBody otherBody, CharacterController otherController, ContactType contactType, bool trigger)
	{
		if (contactType != ContactType.Found)
			return;

		if (otherBody != null && !(otherBody.entity is Player))
		{
			hit = true;
			hitTarget = otherBody.entity as Entity;

			OntoNewHorizons.instance.world.addEntity(new Explosion(shooter), position, rotation);
