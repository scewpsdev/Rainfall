...
internal class Creature : Entity, Hittable
{
	...
	void updateMovement()
	{
		if (state != CreatureState.Dead)
		{
			...
			if (verticalVelocity < 0.01f)
			{
				...
				int numHits = Physics.SweepSphere(0.3f, position + new Vector3(0.0f, 0.1f, 0.0f), Vector3.Down, 0.2f, hits, QueryFilterFlags.Static | QueryFilterFlags.Dynamic);
	...
	public override void draw(GraphicsDevice graphics)
	{
		...
		if (renderHealthbar)
		{
			...
			if (screenPosition.x != -1 && screenPosition.y != -1)
			{
				...
				// TODO fix
				Span<byte> healthStr = stackalloc byte[32];
				StringUtils.WriteInteger(healthStr, stats.health);
				Renderer.DrawText(screenPosition.x - width / 2, screenPosition.y - 20, 1.0f, healthStr, healthbarTextFont, 0xffffffff);
			}
		}
