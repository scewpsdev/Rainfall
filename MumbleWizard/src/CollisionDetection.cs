...
public static class CollisionDetection
{
	...
	{
		grid = new List<Entity>[level.width * level.height / 16];
		for (int i = 0; i < grid.Length; i++)
			grid[i] = new List<Entity>();
	}
	...
	public static void Update(Level level)
	{
		...
		{
			Vector2i tile = (Vector2i)entity.position;
	...
	public static List<Entity> OverlapEntities(Vector2 position, Vector2 size, Level level)
	{
		...
		Vector2i tile = (Vector2i)position;
		Vector2i chunk = tile / 4;
		Vector2i min = chunk - 1;
		Vector2i max = chunk + 1;

		for (int y = min.y; y <= max.y; y++)
		{
			for (int x = min.x; x <= max.x; x++)
			{
				List<Entity> entities = grid[x + y * level.width / 4];
		...
		result.Sort((Entity a, Entity b) =>
		{
			float d1 = (a.position - (position + 0.5f * size)).length;
			float d2 = (b.position - (position + 0.5f * size)).length;
