...
public class MainMenuState : State
{
		...
		if (selection != -1)
		{
			switch (selection)
			{
				...
				case 1: // Daily Run
					...
					ARPG.instance.pushState(new GameState(-1, seed.ToString(), false, true));
					break;
				...
				case 5: // Quit
					ARPG.instance.popState();
					ARPG.instance.terminate();
					break;
	...
	void saveSelect()
	{
		...
		if (selection != -1)
		{
			...
			ARPG.instance.pushState(new GameState(selection, null));
		}
	}
	...
	public override void onKeyEvent(KeyCode key, KeyModifier modifiers, bool down)
	{
		...
		else if (screen == MainMenuScreen.CustomRunSettings)
		{
			...
			{
				ARPG.instance.pushState(new GameState(-1, customRunSeedStr.ToString(), true));
				screen = MainMenuScreen.Main;
			}
