...
public class ParallaxObject : Entity
{
	...
	public override void render()
	{
		...
		Renderer.DrawSprite(position.x + rect.min.x, position.y + rect.min.y, z, rect.size.x, rect.size.y, 0, sprite, false, Vector4.One);
		Renderer.DrawSpriteSolid(position.x + rect.min.x, position.y + rect.min.y, z + 0.01f, rect.size.x, rect.size.y, 0, sprite, false, MathHelper.VectorToARGB(new Vector4(GameState.instance.level.fogColor, 1 - fog)));
	}
	...
	public static Vector3 ParallaxEffect(Vector2 vertex, float layer)
	{
		...
		return new Vector3(vertex, -LayerToZ(layer));
	}
