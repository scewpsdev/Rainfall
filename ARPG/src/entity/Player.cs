...
public class Player : Entity, Hittable, StatusEffectReceiver
{
		...
		if (isAlive && numOverlaysOpen == 0)
		{
			...
			else if (Settings.game.aimMode == AimMode.Directional)
			{
				if (Input.cursorHasMoved)
				{
					...
					if (MathF.Abs(Input.cursorPosition.x - playerScreenPos.x) > maxCursorDistance * 16 * ARPG.instance.scale ||
						MathF.Abs(Input.cursorPosition.y - playerScreenPos.y) > maxCursorDistance * 16 * ARPG.instance.scale)
					{
						int x = Math.Clamp(Input.cursorPosition.x, (int)MathF.Round(playerScreenPos.x - maxCursorDistance * 16 * ARPG.instance.scale), (int)MathF.Round(playerScreenPos.x + maxCursorDistance * 16 * ARPG.instance.scale));
						int y = Math.Clamp(Input.cursorPosition.y, (int)MathF.Round(playerScreenPos.y - maxCursorDistance * 16 * ARPG.instance.scale), (int)MathF.Round(playerScreenPos.y + maxCursorDistance * 16 * ARPG.instance.scale));
	...
	unsafe void updateAnimation()
	{
		...
		for (int i = 0; i < items.Count; i++)
		{
			if (items[i].ingameSprite != null)
			{
				...
				items[i].ingameSprite.position /= 2;
			}
		}
	...
	public override void render()
	{
		...
		if (isVisible)
		{
			...
			if (show)
				Renderer.DrawSprite(snappedPosition.x - 1.0f, snappedPosition.y - 0.5f, 2, isDucked && !isClimbing ? 1 : 2, sprite, direction == -1, 0xFFFFFFFF);
