...
public class Camera
{
	...
	public void updateControls(EditorInstance instance)
	{
		...
		if (ImGui.IsWindowHovered())
		{
			if (ImGui.IsMouseButtonDown(MouseButton.Left))
			{
				if (ImGui.IsKeyDown(KeyCode.Alt))
				{
					if (ImGui.IsKeyDown(KeyCode.Shift))
					{
						...
						float multiplier = 2 * distance * MathF.Tan(0.5f * Mathf.ToRadians(FOV));
			...
			{
				pitch = Mathf.LerpAngle((pitch), rotationTarget.x, 16.0f * Time.deltaTime);
				yaw = Mathf.LerpAngle(yaw, rotationTarget.y, 16.0f * Time.deltaTime);
				if (Mathf.CompareAngles(pitch, rotationTarget.x, 0.01f) && Mathf.CompareAngles(yaw, rotationTarget.y, 0.01f))
				{
	...
	{
		if (orthographic)
			//return Matrix.CreatePerspective(Mathf.ToRadians(3), aspect, NEAR, FAR) * Matrix.CreateTranslation(0.0f, 0.0f, -40);
			return Matrix.CreateOrthographic(distance * 2 * aspect, distance * 2, -100, +100);
		else
			return Matrix.CreatePerspective(Mathf.ToRadians(FOV), aspect, NEAR, FAR);
	}
