...
namespace Rainfall
{
	...
	[StructLayout(LayoutKind.Sequential)]
	public unsafe struct PhysicsHit
	{
	...
	public static unsafe class Physics
	{
		...
		public static unsafe MeshCollider CreateMeshCollider(MeshData* mesh, Matrix transform)
		{
		...
		public static int Raycast(Vector3 origin, Vector3 direction, float distance, Span<PhysicsHit> hits, QueryFilterFlags filterData = QueryFilterFlags.Default, uint filterMask = 1)
		{
			unsafe
			{
				fixed (PhysicsHit* data = hits)
					return Physics_Raycast(origin, direction, distance, data, hits.Length, filterData, filterMask);
			}
		}

		public static int Raycast(Vector3 origin, Vector3 direction, float distance, PhysicsHit[] hits, QueryFilterFlags filterData = QueryFilterFlags.Default, uint filterMask = 1)
		{
			unsafe
			{
				fixed (PhysicsHit* data = hits)
		...
		public static PhysicsHit? Raycast(Vector3 origin, Vector3 direction, float distance, QueryFilterFlags filterData = QueryFilterFlags.Default, uint filterMask = 1)
		{
			if (Physics_RaycastCheck(origin, direction, distance, out PhysicsHit hit, filterData, filterMask) != 0)
				return hit;
			return null;
		}

		public static int SweepBox(Vector3 halfExtents, Vector3 position, Quaternion rotation, Vector3 direction, float distance, Span<PhysicsHit> hits, QueryFilterFlags filterData = QueryFilterFlags.Default, uint filterMask = 1)
		{
			unsafe
			{
				fixed (PhysicsHit* data = hits)
		...
		public static int SweepBox(Vector3 halfExtents, Vector3 position, Quaternion rotation, Vector3 direction, float distance, PhysicsHit[] hits, QueryFilterFlags filterData = QueryFilterFlags.Default, uint filterMask = 1)
		{
			unsafe
			{
				fixed (PhysicsHit* data = hits)
		...
		public static int SweepSphere(float radius, Vector3 position, Vector3 direction, float distance, Span<PhysicsHit> hits, QueryFilterFlags filterData = QueryFilterFlags.Default, uint filterMask = 1)
		{
			unsafe
			{
				fixed (PhysicsHit* data = hits)
		...
		public static int SweepSphere(float radius, Vector3 position, Vector3 direction, float distance, PhysicsHit[] hits, QueryFilterFlags filterData = QueryFilterFlags.Default, uint filterMask = 1)
		{
			unsafe
			{
				fixed (PhysicsHit* data = hits)
		...
		public static PhysicsHit? SweepSphere(float radius, Vector3 position, Vector3 direction, float distance, QueryFilterFlags filterData = QueryFilterFlags.Default, uint filterMask = 1)
		{
		...
		public static int SweepCapsule(float radius, float height, Vector3 position, Quaternion rotation, Vector3 direction, float distance, Span<PhysicsHit> hits, QueryFilterFlags filterData = QueryFilterFlags.Default, uint filterMask = 1)
		{
			unsafe
			{
				fixed (PhysicsHit* data = hits)
		...
		public static int SweepCapsule(float radius, float height, Vector3 position, Quaternion rotation, Vector3 direction, float distance, PhysicsHit[] hits, QueryFilterFlags filterData = QueryFilterFlags.Default, uint filterMask = 1)
		{
			unsafe
			{
				fixed (PhysicsHit* data = hits)
		...
		public static int OverlapBox(Vector3 halfExtents, Vector3 position, Quaternion rotation, Span<PhysicsHit> hits, QueryFilterFlags filterData = QueryFilterFlags.Default, uint filterMask = 1)
		{
			unsafe
			{
				fixed (PhysicsHit* data = hits)
		...
		public static int OverlapBox(Vector3 halfExtents, Vector3 position, Quaternion rotation, PhysicsHit[] hits, QueryFilterFlags filterData = QueryFilterFlags.Default, uint filterMask = 1)
		{
			unsafe
			{
				fixed (PhysicsHit* data = hits)
		...
		public static int OverlapSphere(float radius, Vector3 position, Span<PhysicsHit> hits, QueryFilterFlags filterData = QueryFilterFlags.Default, uint filterMask = 1)
		{
			unsafe
			{
				fixed (PhysicsHit* data = hits)
		...
		public static int OverlapSphere(float radius, Vector3 position, PhysicsHit[] hits, QueryFilterFlags filterData = QueryFilterFlags.Default, uint filterMask = 1)
		{
			unsafe
			{
				fixed (PhysicsHit* data = hits)
		...
		public static int OverlapCapsule(float radius, float height, Vector3 position, Quaternion rotation, Span<PhysicsHit> hits, QueryFilterFlags filterData = QueryFilterFlags.Default, uint filterMask = 1)
		{
			unsafe
			{
				fixed (PhysicsHit* data = hits)
		...
		public static int OverlapCapsule(float radius, float height, Vector3 position, Quaternion rotation, PhysicsHit[] hits, QueryFilterFlags filterData = QueryFilterFlags.Default, uint filterMask = 1)
		{
			unsafe
			{
				fixed (PhysicsHit* data = hits)
		...
		[DllImport(Native.Native.DllName, CallingConvention = CallingConvention.Cdecl)]
		internal static extern void Physics_RigidBodySetActive(RigidBodyData* body, byte active);
		...
		[DllImport(Native.Native.DllName, CallingConvention = CallingConvention.Cdecl)]
		internal static extern void Physics_RigidBodyScale(RigidBodyData* body, float scale);
		...
		[DllImport(Native.Native.DllName, CallingConvention = CallingConvention.Cdecl)]
		internal static extern void Physics_RigidBodyGetWorldAABB(RigidBodyData* body, out Vector3 position, out Vector3 size);
		...
		[DllImport(Native.Native.DllName, CallingConvention = CallingConvention.Cdecl)]
		internal static extern unsafe int Physics_Raycast(Vector3 origin, Vector3 direction, float maxDistance, PhysicsHit* hits, int maxHits, QueryFilterFlags filterData, uint filterMask);

		[DllImport(Native.Native.DllName, CallingConvention = CallingConvention.Cdecl)]
		internal static extern unsafe byte Physics_RaycastCheck(Vector3 origin, Vector3 direction, float maxDistance, out PhysicsHit hit, QueryFilterFlags filterData, uint filterMask);

		[DllImport(Native.Native.DllName, CallingConvention = CallingConvention.Cdecl)]
		internal static extern unsafe int Physics_SweepBox(Vector3 halfExtents, Vector3 position, Quaternion rotation, Vector3 direction, float maxDistance, PhysicsHit* hits, int maxHits, QueryFilterFlags filterData, uint filterMask);

		[DllImport(Native.Native.DllName, CallingConvention = CallingConvention.Cdecl)]
		internal static extern unsafe int Physics_SweepSphere(float radius, Vector3 position, Quaternion rotation, Vector3 direction, float maxDistance, PhysicsHit* hits, int maxHits, QueryFilterFlags filterData, uint filterMask);

		[DllImport(Native.Native.DllName, CallingConvention = CallingConvention.Cdecl)]
		internal static extern unsafe int Physics_SweepCapsule(float radius, float height, Vector3 position, Quaternion rotation, Vector3 direction, float maxDistance, PhysicsHit* hits, int maxHits, QueryFilterFlags filterData, uint filterMask);

		[DllImport(Native.Native.DllName, CallingConvention = CallingConvention.Cdecl)]
		internal static extern unsafe int Physics_OverlapBox(Vector3 halfExtents, Vector3 position, Quaternion rotation, PhysicsHit* hits, int maxHits, QueryFilterFlags filterData, uint filterMask);

		[DllImport(Native.Native.DllName, CallingConvention = CallingConvention.Cdecl)]
		internal static extern unsafe int Physics_OverlapSphere(float radius, Vector3 position, PhysicsHit* hits, int maxHits, QueryFilterFlags filterData, uint filterMask);

		[DllImport(Native.Native.DllName, CallingConvention = CallingConvention.Cdecl)]
		internal static extern unsafe int Physics_OverlapCapsule(float radius, float height, Vector3 position, Quaternion rotation, PhysicsHit* hits, int maxHits, QueryFilterFlags filterData, uint filterMask);
	}
}
