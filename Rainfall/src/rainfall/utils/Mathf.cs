...
namespace Rainfall
{
	public struct Mathf
	{
		...
		public static float MoveTowards(float v, float dst, float d)
		{
			if (dst > v)
		...
		public static Vector2 RandomVector2(Vector2 min, Vector2 max, Random random = null)
		{
			if (random == null)
				random = Random.Shared;
			return new Vector2(RandomFloat(min.x, max.x, random), RandomFloat(min.y, max.y, random));
		}
		...
		{
			return Quaternion.AxisAngle(RandomVector3(-1, 1, random).normalized, random.NextSingle() * MathF.PI * 2);
		}
		...
		public static int RollDice(int count, int sides, Random random = null)
		{
			if (random == null)
				random = Random.Shared;
			...
			{
				result += RandomInt(1, sides, random);
			}
			return result;
		}
		...
		public static T ChooseRandom<T>(List<T> list, Func<T, float> getRarity, Random random = null)
		{
			if (random == null)
				random = Random.Shared;
			float raritySum = 0;
