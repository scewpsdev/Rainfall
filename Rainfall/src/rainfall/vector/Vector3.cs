...
namespace Rainfall
{
	...
	public struct Vector3
	{
		...
		public float min => MathF.Min(x, MathF.Min(y, z));
		public float max => MathF.Max(x, MathF.Max(y, z));
		...
		{
			return new Vector3(
				(1.0f - MathF.Min(t, 1)) * a.x + MathF.Min(t, 1) * b.x,
				(1.0f - MathF.Min(t, 1)) * a.y + MathF.Min(t, 1) * b.y,
				(1.0f - MathF.Min(t, 1)) * a.z + MathF.Min(t, 1) * b.z
			);
		}

		public static Vector3 Slerp(Vector3 a, Vector3 b, float t)
		{
			float d = MathF.Min(Dot(a, b), 1);
			float theta = MathF.Acos(d);
			if (theta == 0 || theta == MathF.PI)
				return a;
			return a * MathF.Sin((1 - t) * theta) / MathF.Sin(theta) + b * MathF.Sin(t * theta) / MathF.Sin(theta);
		}
