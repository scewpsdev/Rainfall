...
namespace Rainfall
{
	...
	public class GraphicsDevice
	{
		...
		public Texture createTexture(int width, int height, TextureFormat format, bool hasMips, int numLayers, VideoMemory memory, ulong flags = 0)
		{
			ushort handle = Native.Graphics.Graphics_CreateTextureImmutable(width, height, format, (byte)(hasMips ? 1 : 0), numLayers, flags, memory.memoryHandle, out TextureInfo info);
			if (handle != ushort.MaxValue)
		...
		*/
		...
		public unsafe uint readTexture(Texture texture, GCHandle data)
		{
			return Native.Graphics.Graphics_ReadTexture(currentPass, texture.handle, (byte*)data.AddrOfPinnedObject());
		}

		public uint getFrameIndex()
		{
			return Native.Graphics.Graphics_GetFrameIndex();
		}
