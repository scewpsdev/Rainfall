...
namespace Rainfall
{
	...
	public unsafe struct MeshData
	{
		...
		byte _triangleStrip;
		public bool triangleStrip { get => _triangleStrip != 0; set { _triangleStrip = (byte)(value ? 1 : 0); } }
		byte _renderBackfaces;
		public bool renderBackfaces { get => _renderBackfaces != 0; set { _renderBackfaces = (byte)(value ? 1 : 0); } }
		...
		public ushort texcoordBufferID => texcoordBuffer;
	...
	public unsafe struct CustomProperty
	{
		...
		{
			if (type == CustomPropertyType.Double)
			{
				fixed (ulong* dataPtr = &data)
	...
	public unsafe struct NodeData
	{
		...
		internal CustomProperty* properties;
		...
		public CustomProperty* getProperty(string name)
		{
			...
			{
				if (new string((sbyte*)properties[i].name) == name)
					return &properties[i];
			}
			return null;
		}
	...
	public class Model
	{
		...
		public unsafe Model(int numParts, Span<int> numVertices, Span<PositionNormalTangent> vertices, Span<Vector2> uvs, Span<int> numIndices, Span<int> indices)
		{
			fixed (int* numVerticesPtr = numVertices)
			fixed (PositionNormalTangent* verticesPtr = vertices)
			fixed (Vector2* uvsPtr = uvs)
			fixed (int* numIndicesPtr = numIndices)
			fixed (int* indicesPtr = indices)
			{
				scene = Model_CreateFromParts(numParts, numVerticesPtr, verticesPtr, uvsPtr, numIndicesPtr, indicesPtr);
			}
			skeleton = null;
			ownsScene = true;
		}
		...
		public unsafe void createMaterials(int numMaterials)
		{
			Model_CreateMaterials(scene, numMaterials);
		}

		public unsafe void applyMaterials()
		{
			Model_ApplyMaterials(scene);
		}
		...
		public unsafe NodeData* getNodeData(int index)
		{
			if (index < scene->numNodes)
		...
		public unsafe IntPtr getMaterialHandleForMesh(int meshID)
		{
			MeshData* mesh = &scene->meshes[meshID];
			return mesh->materialID != -1 ? Material.Material_GetForData(getMaterialData(mesh->materialID)) : Material.Material_GetDefault();
		}

		public unsafe IntPtr getMaterialHandleForMeshNode(Node node, int meshIdx = 0)
		{
			return getMaterialHandleForMesh(scene->nodes[node.id].meshes[meshIdx]);
		}
		...
		[DllImport(Native.Native.DllName, CallingConvention = CallingConvention.Cdecl)]
		static extern unsafe SceneData* Model_CreateFromParts(int numParts, int* numVertices, PositionNormalTangent* vertices, Vector2* uvs, int* numIndices, int* indices);

		[DllImport(Native.Native.DllName, CallingConvention = CallingConvention.Cdecl)]
		static extern unsafe void Model_CreateMaterials(SceneData* scene, int numMaterials);

		[DllImport(Native.Native.DllName, CallingConvention = CallingConvention.Cdecl)]
		static extern unsafe void Model_ApplyMaterials(SceneData* scene);
