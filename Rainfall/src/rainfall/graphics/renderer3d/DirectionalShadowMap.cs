...
public class DirectionalShadowMap
{
	...
	public DirectionalShadowMap(int resolution, DirectionalLight light, GraphicsDevice graphics)
	{
		this.light = light;
		...
		for (int i = 0; i < NUM_SHADOW_CASCADES; i++)
		{
			...
			{
				new RenderTargetAttachment(resolution, resolution, TextureFormat.D24, (ulong)TextureFlags.RenderTarget | (uint)SamplerFlags.UClamp | (uint)SamplerFlags.VClamp | (uint)SamplerFlags.CompareLEqual)
			});
		}
	}
	...
	void calculateViewTransform(Vector3 cameraPosition, Quaternion cameraRotation, float cameraFov, float cameraAspect, float near, float far, out Matrix lightProjection, out Matrix lightView)
	{
		...
		float halfHeight = MathF.Tan(Mathf.ToRadians(cameraFov * 0.5f));
