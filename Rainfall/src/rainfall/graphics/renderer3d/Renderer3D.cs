...
namespace Rainfall
{
	...
	public static unsafe class Renderer
	{
		...
		public static void Init(int width, int height, GraphicsDevice graphics)
		{
			...
			GUI.Init(width, height, graphics);
		}
		...
		public static unsafe void DrawMesh(MeshData* mesh, MaterialData* material, Matrix transform, Animator animator = null, bool isOccluder = false, bool renderShadowMap = true)
		{
			IntPtr materialHandle = material != null && mesh->materialID != -1 ? Material.Material_GetForData(material) : Material.Material_GetDefault();
		...
		public static void DrawMesh(MeshData* mesh, Material material, Matrix transform, Animator animator = null, bool isOccluder = false, bool renderShadowMap = true)
		{
		...
		public static void DrawModelNode(Model model, Node node, Material material, Matrix transform, Animator animator = null, bool isOccluder = false, bool renderShadowMap = true)
		{
			if (node.meshes != null)
			{
				...
				if (animator != null)
				{
					meshTransform = transform * animator.getNodeTransform(node);
				}
				else
				{
				...
				{
					MeshData* mesh = model.getMeshData(node.meshes[i]);
					IntPtr materialHandle = material != null ? material.handle : mesh->materialID != -1 ? Material.Material_GetForData(model.getMaterialData(mesh->materialID)) : Material.Material_GetDefault();
					Renderer3D_DrawMesh(mesh, meshTransform, materialHandle, animator != null ? animator.handle : IntPtr.Zero, (byte)(isOccluder ? 1 : 0), (byte)(renderShadowMap ? 1 : 0));
				}
			}
			if (node.children != null)
			{
				...
				{
					DrawModelNode(model, node.children[i], material, transform, animator, isOccluder, renderShadowMap);
				}
			}
		}
		...
		public static void DrawModel(Model model, Matrix transform, Material material, Animator animator = null, bool isOccluder = false, bool renderShadowMap = true)
		{
			if (model.skeleton != null)
				DrawModelNode(model, model.skeleton.rootNode, material, transform, animator, isOccluder, renderShadowMap);
			else
			{
				Renderer3D_DrawScene(model.scene, transform, animator != null ? animator.handle : IntPtr.Zero, (byte)(isOccluder ? 1 : 0), (byte)(renderShadowMap ? 1 : 0));
			}
		...
		{
			DrawModelNode(model, model.skeleton.rootNode, null, transform, animator, isOccluder, renderShadowMap);
		}
		...
		*/
		...
		public static void DrawCustomGeometry(Span<VertexBuffer> vertexBuffers, IndexBuffer indexBuffer, Matrix transform, Material material, PrimitiveType primitiveType = PrimitiveType.Triangle, BlendState blendState = BlendState.Default)
		{
			...
			fixed (ushort* vertexBufferHandlesPtr = vertexBufferHandles)
				Renderer3D_DrawCustomGeometry(vertexBuffers.Length, vertexBufferHandlesPtr, 0, indexBuffer.handle, null, primitiveType, blendState, transform, material.handle);
		}
		...
		public static void DrawCustomGeometry(Span<DynamicVertexBuffer> vertexBuffers, IndexBuffer indexBuffer, Matrix transform, Material material, PrimitiveType primitiveType = PrimitiveType.Triangle, BlendState blendState = BlendState.Default)
		{
			...
			for (int i = 0; i < vertexBuffers.Length; i++)
				vertexBufferHandles[i] = vertexBuffers[i].handle;
			fixed (ushort* vertexBufferHandlesPtr = vertexBufferHandles)
				Renderer3D_DrawCustomGeometry(vertexBuffers.Length, vertexBufferHandlesPtr, 1, indexBuffer != null ? indexBuffer.handle : ushort.MaxValue, null, primitiveType, blendState, transform, material.handle);
		}

		public static void DrawCustomGeometry(Span<VertexBuffer> vertexBuffers, IndexBuffer indexBuffer, InstanceBufferData instanceBuffer, Matrix transform, Material material, PrimitiveType primitiveType = PrimitiveType.Triangle, BlendState blendState = BlendState.Default)
		{
			Span<ushort> vertexBufferHandles = stackalloc ushort[8];
			for (int i = 0; i < vertexBuffers.Length; i++)
				vertexBufferHandles[i] = vertexBuffers[i].handle;
			fixed (ushort* vertexBufferHandlesPtr = vertexBufferHandles)
				Renderer3D_DrawCustomGeometry(vertexBuffers.Length, vertexBufferHandlesPtr, 0, indexBuffer.handle, &instanceBuffer, primitiveType, blendState, transform, material.handle);
		}

		public static void DrawCustomGeometry(Span<DynamicVertexBuffer> vertexBuffers, IndexBuffer indexBuffer, InstanceBufferData instanceBuffer, Matrix transform, Material material, PrimitiveType primitiveType = PrimitiveType.Triangle, BlendState blendState = BlendState.Default)
		{
			Span<ushort> vertexBufferHandles = stackalloc ushort[8];
			for (int i = 0; i < vertexBuffers.Length; i++)
				vertexBufferHandles[i] = vertexBuffers[i].handle;
			fixed (ushort* vertexBufferHandlesPtr = vertexBufferHandles)
				Renderer3D_DrawCustomGeometry(vertexBuffers.Length, vertexBufferHandlesPtr, 1, indexBuffer != null ? indexBuffer.handle : ushort.MaxValue, &instanceBuffer, primitiveType, blendState, transform, material.handle);
		}

		public static void DrawCustomGeometry(Span<ushort> vertexBuffers, ushort indexBuffer, InstanceBufferData instanceBuffer, Matrix transform, Material material, PrimitiveType primitiveType = PrimitiveType.Triangle, BlendState blendState = BlendState.Default)
		{
			fixed (ushort* vertexBufferHandlesPtr = vertexBuffers)
				Renderer3D_DrawCustomGeometry(vertexBuffers.Length, vertexBufferHandlesPtr, 0, indexBuffer, &instanceBuffer, primitiveType, blendState, transform, material.handle);
		}
		...
		public static void DrawDebugSphere(float radius, Matrix transform, uint color)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				Quaternion ringRot = j == 0 ? Quaternion.Identity : j == 1 ? Quaternion.AxisAngle(Vector3.UnitX, MathF.PI * 0.5f) : Quaternion.AxisAngle(Vector3.UnitZ, MathF.PI * 0.5f);

				for (int k = 0; k < segmentCount; k++)
				{
					Vector3 vertex0 = transform * (ringRot * Quaternion.AxisAngle(Vector3.Up, k / (float)segmentCount * 2 * MathF.PI) * new Vector3(0.0f, 0.0f, radius));
					Vector3 vertex1 = transform * (ringRot * Quaternion.AxisAngle(Vector3.Up, (k + 1) / (float)segmentCount * 2 * MathF.PI) * new Vector3(0.0f, 0.0f, radius));
		...
		public static void DrawDebugCapsule(float radius, float height, Matrix transform, uint color)
		{
			...
			for (int k = 0; k < segmentCount; k++)
			{
				Vector3 vertex0 = transform * (new Vector3(0.0f, height * 0.5f - radius, 0.0f) + Quaternion.AxisAngle(Vector3.Up, k / (float)segmentCount * 2 * MathF.PI) * new Vector3(0.0f, 0.0f, radius));
				Vector3 vertex1 = transform * (new Vector3(0.0f, height * 0.5f - radius, 0.0f) + Quaternion.AxisAngle(Vector3.Up, (k + 1) / (float)segmentCount * 2 * MathF.PI) * new Vector3(0.0f, 0.0f, radius));
			...
			for (int k = 0; k < segmentCount; k++)
			{
				Quaternion ringRot = Quaternion.AxisAngle(Vector3.UnitZ, MathF.PI * 0.5f);

				Vector3 vertex0 = transform * ((k < segmentCount / 2 ? 1 : -1) * new Vector3(0.0f, height * 0.5f - radius, 0.0f) + ringRot * Quaternion.AxisAngle(Vector3.Up, k / (float)segmentCount * 2 * MathF.PI) * new Vector3(0.0f, 0.0f, radius));
				Vector3 vertex1 = transform * ((k < segmentCount / 2 ? 1 : -1) * new Vector3(0.0f, height * 0.5f - radius, 0.0f) + ringRot * Quaternion.AxisAngle(Vector3.Up, (k + 1) / (float)segmentCount * 2 * MathF.PI) * new Vector3(0.0f, 0.0f, radius));
			...
			for (int k = 0; k < segmentCount; k++)
			{
				Quaternion ringRot = Quaternion.AxisAngle(Vector3.UnitY, MathF.PI * 0.5f) * Quaternion.AxisAngle(Vector3.UnitZ, MathF.PI * 0.5f);
			...
			for (int k = 0; k < 4; k++)
			{
				Vector3 vertex0 = transform * (new Vector3(0.0f, -height * 0.5f + radius, 0.0f) + Quaternion.AxisAngle(Vector3.Up, k / 4.0f * MathF.PI * 2) * new Vector3(0.0f, 0.0f, radius));
				Vector3 vertex1 = transform * (new Vector3(0.0f, height * 0.5f - radius, 0.0f) + Quaternion.AxisAngle(Vector3.Up, k / 4.0f * MathF.PI * 2) * new Vector3(0.0f, 0.0f, radius));
		...
		public static void SetCamera(Vector3 position, Quaternion rotation, float fov, float aspect, float near, float far)
		{
			...
			Matrix projection = Matrix.CreatePerspective(Mathf.ToRadians(fov), aspect, near, far);
		...
		[DllImport(Native.Native.DllName, CallingConvention = CallingConvention.Cdecl)]
		extern static void Renderer3D_DrawCustomGeometry(int numVertexBuffers, ushort* vertexBuffers, byte dynamicVertexBuffers, ushort indexBuffer, InstanceBufferData* instanceBuffer, PrimitiveType primitiveType, BlendState blendState, Matrix transform, IntPtr material);
