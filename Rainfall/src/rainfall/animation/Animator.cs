...
namespace Rainfall
{
	...
	public class Animator
	{
		...
		public void getRootMotion(AnimationState animation, out Vector3 position, out Quaternion rotation, out bool hasLooped)
		{
			if (animation != null && currentUpdateTime > lastAnimUpdateTime)
			{
				//float dt = (currentUpdateTime - lastAnimUpdateTime) / 1e9f;
				Matrix displacement = animation.layers[0].rootMotionDisplacement;
				position = displacement.translation;
				rotation = displacement.rotation;
				hasLooped = animation.layers[0].animationTimer < lastRootMotionTimer;
			}
			else
			{
			...
			lastRootMotionTimer = animation.layers[0].animationTimer;
		}
