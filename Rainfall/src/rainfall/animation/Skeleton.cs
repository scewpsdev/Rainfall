...
namespace Rainfall
{
	public class Node
	{
		...
		public unsafe int getPropertyCount(Model model) => model.getNodeData(id)->numProperties;

		public unsafe bool getProperty(int idx, Model model, out CustomProperty* property)
		{
			NodeData* node = model.getNodeData(id);
			if (node != null && idx < node->numProperties)
			{
				property = &node->properties[idx];
				return true;
			}
			property = null;
			return false;
		...
		public unsafe bool getProperty(string name, Model model)
		{
			NodeData* node = model.getNodeData(id);
			CustomProperty* property = node->getProperty(name);
			if (property != null)
				return true;
			return false;
		}
