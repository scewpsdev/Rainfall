...
namespace Rainfall
{
	...
	namespace Native
	{
		...
		internal static unsafe class Graphics
		{
			...
			[DllImport(Native.DllName, CallingConvention = CallingConvention.Cdecl)]
			internal static extern ushort Graphics_CreateTextureImmutable(int width, int height, TextureFormat format, byte hasMips, int numLayers, ulong flags, IntPtr memory, out TextureInfo info);
			...
			[DllImport(Native.DllName, CallingConvention = CallingConvention.Cdecl)]
			internal static extern uint Graphics_ReadTexture(int pass, ushort texture, byte* data);

			[DllImport(Native.DllName, CallingConvention = CallingConvention.Cdecl)]
			internal static extern uint Graphics_GetFrameIndex();
