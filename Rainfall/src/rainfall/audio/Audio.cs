...
namespace Rainfall
{
	...
	public static class Audio
	{
		...
		public static uint Play(Sound sound, Vector3 position, float gain = 1, float pitch = 1, float rolloff = 1.0f)
		{
			return Audio_SourcePlay(sound.handle, Time.deltaTime, position, gain, pitch, rolloff);
		}

		public static uint Play(Sound[] sounds, Vector3 position, float gain = 1, float pitch = 1, float rolloff = 1.0f)
		{
			return Play(sounds[Random.Shared.Next() % sounds.Length], position, gain, pitch, rolloff);
		}

		public static uint PlayOrganic(Sound sound, Vector3 position, float gain = 1.0f, float pitch = 1.0f, float gainVariation = 0.2f, float pitchVariation = 0.25f, float rolloff = 1.0f)
		{
			float gainFactor = Mathf.RandomFloat(1.0f - gainVariation, 1.0f + gainVariation);
			float pitchFactor = Mathf.RandomFloat(1.0f - pitchVariation, 1.0f + pitchVariation);
			return Audio_SourcePlay(sound.handle, Time.deltaTime, position, gainFactor * gain, pitchFactor * pitch, rolloff);
		}

		public static uint PlayOrganic(Sound[] sounds, Vector3 position, float gain = 1.0f, float pitch = 1.0f, float gainVariation = 0.0f, float pitchVariation = 0.25f, float rolloff = 1.0f)
		{
